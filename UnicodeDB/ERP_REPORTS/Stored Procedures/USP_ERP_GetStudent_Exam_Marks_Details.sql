CREATE PROCEDURE [ERP_REPORTS].[USP_ERP_GetStudent_Exam_Marks_Details]  
    @BrandID INT,  
    @SessionID INT,  
    @SchoolGroupID INT,  
    --@ClassID INT, 
...
FROM dbo.ERP_SplitString(@strSection, ',');
    ...
    SELECT DISTINCT    
        ...
        ESS.dtStartDate AS ExamStartDate,  
        ESS.dtEndDate AS ExamEndDate,  
        CONCAT(ESM.tmSlotStartTime, '-', ESM.tmSlotEndTime) AS ExamSlot,  
        SM.S_Subject_Name AS Subject,  
        ESS.dcFullMarks AS Fullmarks,  
        ESS.dcMainExamFullMarks AS MainExamFullmarks,  
        ESS.dcInternalFullMarks AS Internal_Full_Marks,  
        ESS.dcOrderallPassMarks AS Overall_Pass_Marks,  
        SSAM.dcObtainedMarks AS StudentMarks,  
        CASE   
            WHEN SSAM.inPaperStatus = 1 THEN 'Active'  
            ELSE 'Hold'   
        END AS PaperStatus,  
        EGMH.stGradeHederName AS ExamGrade,  
        SSAM.inConduct AS Conduct,  
        SSAM.stRemarks AS Remarks  
