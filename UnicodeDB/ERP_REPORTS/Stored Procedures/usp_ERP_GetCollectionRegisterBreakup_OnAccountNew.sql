--EXEC [ERP_REPORTS].[usp_ERP_GetCollectionRegisterBreakup_OnAccountNew]  null,107,NULL,NULL,'2025-04-01','2025-04-14'

CREATE   PROCEDURE [ERP_REPORTS].[usp_ERP_GetCollectionRegisterBreakup_OnAccountNew]      
         
    (          
      @sHierarchyList VARCHAR(MAX)=NULL ,          
      @iBrandID INT ,       
   @iSchoolGroupID varchar(100)=null,      
   --@iClassID INT,      
...
--SELECT Value                                        
--FROM dbo.ERP_SplitString(@iSchoolGroupID, ',');   
-- --step1      
...
--TM.S_Tax_Desc,          
    ...
    ELSE 'Online' END "Mode of Payment",      
        ...
        FROM    T_Receipt_Header RH          
                ...
                Left JOIN T_Student_Detail SD ON RH.I_Student_Detail_ID = SD.I_Student_Detail_ID          
                Left JOIN T_Invoice_Child_Detail ICD ON ICD.I_Invoice_Detail_ID = RCD.I_Invoice_Detail_ID          
                Left JOIN T_Fee_Component_Master FCM ON FCM.I_Fee_Component_ID = ICD.I_Fee_Component_ID          
...
Left JOIN T_Center_Hierarchy_Name_Details TCHND ON TCHND.I_Center_ID = RH.I_Centre_Id          
                LEFT JOIN T_PaymentMode_Master PM ON PM.I_PaymentMode_ID = RH.I_PaymentMode_ID          
                left JOIN dbo.T_Invoice_Child_Detail TICD ON TICD.I_Invoice_Detail_ID = RCD.I_Invoice_Detail_ID      
 ...
 --Inner Join #SchoolGroup tsg on tsg.SchoolGroupID=SG.I_School_Group_ID  
    ...
    INNER JOIN T_Class as C on C.I_Class_ID=SGC.I_Class_ID   and C.I_Brand_ID=@iBrandID   
    ...
    CROSS JOIN T_Brand_Master BM      
        WHERE   FCM.I_Brand_ID = @iBrandID and BM.I_Brand_ID=@iBrandID   and RH.I_Invoice_Header_ID is NULL        
    ...
    and (SGC.I_Class_ID in (SELECT Value FROM dbo.ERP_SplitString(@strclass, ','))   
	OR @strclass is null)
    and (SGC.I_School_Group_ID in (SELECT Value FROM dbo.ERP_SplitString(@iSchoolGroupID, ',')) 
	OR @iSchoolGroupID IS NULL)
...
and (SGC.I_Class_ID in (SELECT Value FROM dbo.ERP_SplitString(@strclass, ','))   
	...
	OR @iSchoolGroupID IS NULL)    
                ...
                AND DATEDIFF(dd, @endDate, RH.Dt_Upd_On) <= 0      --and RCD.N_Amount_Paid>0.5          
                AND RH.I_Status = 0          
        GROUP BY  RH.I_Centre_Id,      
    TCHND.S_Center_Name,      
    SD.I_Student_Detail_ID,      
