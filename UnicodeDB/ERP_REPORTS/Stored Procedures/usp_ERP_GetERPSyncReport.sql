...
CREATE PROCEDURE [ERP_REPORTS].[usp_ERP_GetERPSyncReport]        
    (        
...
/*      
    ...
    --tttm.I_Transaction_Nature_ID ,        
        ...
        FROM    ERP.T_Student_Transaction_Details AS tstd        
                ...
                INNER JOIN dbo.T_Fee_Component_Master  FCM ON tttm.I_Fee_Component_ID = FCM.I_Fee_Component_ID        
                INNER JOIN dbo.T_Brand_Master AS tbm ON tttm.I_Brand_ID = tbm.I_Brand_ID        
                INNER JOIN dbo.T_Enquiry_Regn_Detail AS terd ON tstd.I_Enquiry_Regn_ID = terd.I_Enquiry_Regn_ID              WHERE   tstd.I_Enquiry_Regn_ID IS NOT NULL        
                AND tstd.I_Student_ID IS NULL        
                AND tbm.I_Brand_ID = @iBrandID        
                AND DATEDIFF(dd,tstd.Transaction_Date, @dtStartDate) <= 0        
                AND DATEDIFF(dd,tstd.Transaction_Date, @dtEndDate) >= 0        
 ...
 SELECT  'Student' AS [Type] ,        
    ...
    INNER JOIN dbo.T_Centre_Master AS tcm ON tstd.S_Cost_Center = tcm.S_Cost_Center        
        ...
        WHERE   tstd.I_Enquiry_Regn_ID IS NULL        
                ...
                AND DATEDIFF(dd,tstd.Transaction_Date, @dtStartDate) <= 0        
                AND DATEDIFF(dd,tstd.Transaction_Date, @dtEndDate) >= 0      
     AND tcm.I_Centre_Id IN (SELECT FGCFR.centerID  FROM  dbo.fnGetCentreIdByBrandId(@iBrandID) fgcfr)     
               -- AND tcm.I_Centre_Id IN (SELECT fgcfr.centerID FROM dbo.fnGetCentersForReports(@sHierarchyList, @iBrandID) AS fgcfr)        
    ...
    --tttm.I_Transaction_Nature_ID ,        
        ...
        WHERE   tstd.I_Enquiry_Regn_ID IS NOT NULL        
                ...
                AND DATEDIFF(dd,tstd.Transaction_Date, @dtEndDate) >= 0     
    AND tcm.I_Centre_Id IN (SELECT FGCFR.centerID  FROM  dbo.fnGetCentreIdByBrandId(@iBrandID) fgcfr)    
                --AND tcm.I_Centre_Id IN (SELECT  FGCFR.centerID          
    ...
    S_Center_Name ,        
         ...
         --Select * from #FinalTemp order by S_Student_ID          
               ------SEPERATE-------         
...
SELECT   
    --@cols = STUFF((SELECT DISTINCT ',' + QUOTENAME(S_Transaction_Code)   
    --               FROM #FinalTemp   
    --   ORDER BY S_Transaction_Code  
    --               FOR XML PATH(''), TYPE).value('.', 'NVARCHAR(MAX)'), 1, 1, '');  
  @cols = STUFF((SELECT ',' + QUOTENAME(S_Transaction_Code)                     FROM (SELECT S_Transaction_Code                           FROM #FinalTemp                            GROUP BY S_Transaction_Code) AS Codes                    ORDER BY S_Transaction_Code                     FOR XML PATH(''), TYPE).value('.', 'NVARCHAR(MAX)'), 1, 1, '');  
SET @query = N'  
SELECT Type, S_Brand_Name, S_Center_Name, I_Transaction_Nature_ID, S_Student_ID, S_Student_Name, ' + @cols + '  
FROM   
(  
    SELECT   
        Type,   
...
(  
    SUM(Amount)  
    FOR S_Transaction_Code IN (' + @cols + ')  
) AS PivotTable  
ORDER BY   
    Type, S_Brand_Name, S_Center_Name, S_Student_ID;';  
