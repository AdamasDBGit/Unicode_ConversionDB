--EXEC ERP_REPORTS.usp_GetStudentDetails_TransportBrandAndSession 107,7  
CREATE PROCEDURE ERP_REPORTS.usp_GetStudentDetails_TransportBrandAndSession  
    @brandID INT,  
    @SessionID INT  
AS  
BEGIN  
    ...
    SELECT DISTINCT   
      ...
      --  CONCAT('https://ais-sms.arivoo.in/SMS/Upload/', RD.S_Student_Photo) AS ImagePath,  
        ISNULL(TBRM.S_Route_No,'Not Available') AS Route_Name,  
        ISNULL(TTM.S_PickupPoint_Name,'Not Available') AS Stoppage,  
        ISNULL(TTM.N_Fees,0) AS N_fees,  
        CASE   
