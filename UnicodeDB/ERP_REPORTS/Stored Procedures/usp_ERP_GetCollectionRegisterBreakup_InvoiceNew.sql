CREATE  PROCEDURE [ERP_REPORTS].[usp_ERP_GetCollectionRegisterBreakup_InvoiceNew]          
    (          
      @sHierarchyList VARCHAR(MAX)=NULL ,          
      @iBrandID INT ,       
      @iSchoolGroupID varchar(100)=NULL,      
   --@iClassID INT,      
      @strclass varchar(100)=NULL,----1,2,3      
      @startDate DATE ,          
      @endDate DATE          
...
--TM.S_Tax_Desc,          
   ...
   -- Inner Join #Courses ct on ct.CourseID=SGC.I_Class_ID      
    ...
    AND SCS.I_School_Session_ID=@SessionID   AND
     (SG.I_School_Group_ID in (SELECT Value FROM dbo.ERP_SplitString(@iSchoolGroupID, ',')) or @iSchoolGroupID is null )      
        AND ( C.I_Class_ID in (SELECT Value FROM dbo.ERP_SplitString(@strclass, ',')) or @strclass is null)
