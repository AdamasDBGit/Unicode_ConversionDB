CREATE PROCEDURE [ERP_REPORTS].[usp_ERP_TransferCertificateReportAIS]    
(    
...
INSERT INTO #temp(STUDENT_ID,TC_DATE,NARRATION)    
 ...
 SELECT A.I_Student_Detail_ID,Release_Date,S_Remarks   
 FROM T_Transfer_Certificates A   
 ...
 ON C.I_Student_Detail_ID=B.I_Student_Detail_ID AND B.I_Status=1    
  ...
  DATEDIFF(dd,@dtEnd,Release_Date)<=0    
   ...
   AND C.I_Centre_Id IN (SELECT CenterList.centerID FROM dbo.fnGetCentreIdByBrandId(@iBrandID) CenterList)     
...
(    
  ...
  SELECT A.I_Student_Detail_ID,C.S_Batch_Name FROM T_Student_Detail A   
...
(    
  ...
  SELECT B.I_Student_Detail_ID,A.S_Guardian_Name FROM T_Enquiry_Regn_Detail A   
  INNER JOIN T_Student_Detail B    
  ON A.I_Enquiry_Regn_ID=B.I_Enquiry_Regn_ID WHERE A.I_Centre_Id IN (SELECT CenterList.centerID FROM dbo.fnGetCentreIdByBrandId(@iBrandID) CenterList)     
