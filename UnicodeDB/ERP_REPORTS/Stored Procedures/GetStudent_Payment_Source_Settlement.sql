CREATE PROCEDURE [ERP_REPORTS].[GetStudent_Payment_Source_Settlement]    
    @brandID INT,    
    @SessionID INT,    
    @schoolGroupID INT,    
    --@ClassID INT,    
...
BEGIN    
     ...
     SET NOCOUNT ON;    
--step 1    
IF OBJECT_ID('tempdb..#Courses') IS NULL  BEGIN     CREATE TABLE #Courses (         Id INT IDENTITY(1,1), CourseID INT     )  END     
  ...
  TPM.S_PaymentMode_Name,    
   ...
   I_PaymentMode_ID    
        ...
        WHERE     
            ...
            AND I_Centre_Id = @CenterID  and   DATENAME(MONTH, Dt_Receipt_Date) =@month
