...
CREATE PROCEDURE [ERP_REPORTS].[usp_ERP_GetERPSyncReport_PIVOT]      
    (      
...
SELECT 
    ...
    --               FOR XML PATH(''), TYPE).value('.', 'NVARCHAR(MAX)'), 1, 1, '');
	 @cols = STUFF((SELECT ',' + QUOTENAME(S_Transaction_Code) 
                   FROM (SELECT S_Transaction_Code 
                         FROM #FinalTemp  
                         GROUP BY S_Transaction_Code) AS Codes
                   ORDER BY S_Transaction_Code 
                   FOR XML PATH(''), TYPE).value('.', 'NVARCHAR(MAX)'), 1, 1, '');
SET @query = N'
