CREATE PROCEDURE [ERP_REPORTS].[usp_ERP_GetCollectionRegisterReport_AdhocNew] --195,61,'12/1/2007','1/1/2008','ALL'                  
    (                
...
AS                 
   ...
   --TSBD.I_Status as newStatus,                  
                        ...
                        ISNULL(IP.N_Discount_Amount, 0.00) AS Discount_Amount ,                
        ...
        Null as InstanceChain,                
                        CUM.S_Currency_Code as Currency_Code ,                
                        RH.S_Receipt_No AS Receipt_No ,                
                        convert(Date,RH.Dt_Receipt_Date) AS Receipt_Date ,                
                        --RH.I_Student_Detail_ID as StudentDetailsID ,                
                        --RH.I_Enquiry_Regn_ID as EnquiryID ,                
                        RH.I_Invoice_Header_ID as INvoiceHeaderID ,                
                        RH.I_Receipt_Header_ID as ReceiptHeaderID,                
                        RH.I_Status as Inv_Status ,                
   ISNULL(RH.N_Receipt_Amount, 0.00) AS Receipt_Amount ,                
                        ISNULL(RH.N_Tax_Amount, 0.00) AS Receipt_Tax ,     
      ISNULL(RH.N_Receipt_Amount, 0.00)+ISNULL(RH.N_Tax_Amount, 0.00) as Total_ReceiptAmount,    
                        RT.S_Status_Desc AS Receipt_Type_Desc ,                
                        ERD.S_Form_No AS Form_No ,                
 ...
 RH.Bank_Account_Name ,                
             ...
             END AS Instrument_No ,                
                        RH.Dt_ChequeDD_Date as Chequedate ,                
      ...
      ISNULL(RH.N_Tax_Amount,'0.00') as Tax_Component,      
                        UpdatedBy = LTRIM(ISNULL(TUM.S_First_Name, '') + ' ')                
   ...
   RH.I_PaymentMode_ID) AS ConvenienceCharge ,                
    ...
    Into #CollectionDataALL        
                FROM    dbo.T_Receipt_Header RH WITH ( NOLOCK )                
                        LEFT OUTER JOIN dbo.T_ERP_User UM WITH ( NOLOCK ) ON RH.S_Crtd_By = UM.S_Username                
                        INNER JOIN dbo.fnGetCentreIdByBrandId(@iBrandID) FN1 ON RH.I_Centre_Id = FN1.CenterID                
          ...
          AND tsbd.I_Status IN ( 1, 2 )                
                        ...
                        Left Join T_Student_Detail ESD ON ESD.I_Enquiry_Regn_ID=RH.I_Enquiry_Regn_ID  
      ...
      --                                          ), 0.00) ,         
         ...
         dbo.fnCalculateConvenienceChargeTax(dbo.fnCalculateConvenienceCharge(ISNULL(RH.N_Receipt_Amount,                
                ...
                FROM    dbo.T_Receipt_Header RH WITH ( NOLOCK )                
                        ...
                        LEFT OUTER JOIN dbo.T_Enquiry_Regn_Detail ERD WITH ( NOLOCK ) ON RH.I_Enquiry_Regn_ID = ERD.I_Enquiry_Regn_ID  
                        Left Join T_Student_Detail ESD ON ESD.I_Enquiry_Regn_ID=RH.I_Enquiry_Regn_ID
                        LEFT OUTER JOIN DBO.T_ERP_User TUM ON RH.S_Crtd_By = TUM.S_Username                
                WHERE   CAST(SUBSTRING(CAST(RH.Dt_Upd_On AS VARCHAR), 1, 11) AS DATETIME) BETWEEN @dtStartDate    
      AND                
                                                              @dtEndDate                
                        AND ISNULL(IP.I_Invoice_Header_ID, '') = ISNULL(RH.I_Invoice_Header_ID,                
                                                              '')                
                        AND RH.I_Status = 0      and RH.I_Receipt_Type <>2           
                        --AND ( TSBD.I_Status IS NULL                
                        --      OR TSBD.I_Status = 1                
                        --    )                  
                  
         Select * from #CollectionDataALL        
   UNION ALL        
   SELECT         
    'GRAND TOTAL' AS Student_ID,        
    '' AS Student_Name,        
    ' ' AS S_Invoice_No,        
    ...
    SUM(Receipt_Amount),null,    
 SUM(Total_ReceiptAmount),    
  '', '', '', '', NULL,        
    NULL, NULL      
 , SUM(Tax_Component)      
 , NULL,        
