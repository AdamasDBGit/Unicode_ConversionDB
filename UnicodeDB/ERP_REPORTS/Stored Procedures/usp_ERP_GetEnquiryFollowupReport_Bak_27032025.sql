CREATE PROCEDURE [ERP_REPORTS].[usp_ERP_GetEnquiryFollowupReport_Bak_27032025]      
    (      
      @sHierarchyID VARCHAR(MAX)=NULL ,      
      @StartDate DATE ,      
      @EndDate DATE ,      
      @iBrandID INT ,      
      @iFlag INT      
        ...
        IF ( @iFlag = 0 )      
            BEGIN      
                ...
                FROM    ( SELECT    TCHND.I_Brand_ID ,      
                                    ...
                                    CASE WHEN TERD.I_Enquiry_Status_Code=1 OR TERD.I_Enquiry_Status_Code=3 THEN DATENAME(MONTH,ISNULL(TERD.Enquiry_Date,TERD.Dt_Crtd_On))+' '+CAST(DATEPART(YYYY,ISNULL(TERD.Enquiry_Date,TERD.Dt_Crtd_On)) AS VARCHAR) END AS 
