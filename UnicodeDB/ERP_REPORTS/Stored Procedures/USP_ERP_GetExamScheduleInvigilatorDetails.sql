CREATE PROCEDURE [ERP_REPORTS].[USP_ERP_GetExamScheduleInvigilatorDetails]  
    @BrandID INT,  
    @SessionID INT,  
    @SchoolGroupID INT,  
    @ClassID INT,  
...
BEGIN  
    SELECT DISTINCT   
		...
		Case When SSAM.inPresent=1 Then 'Present' Else 'Absent' End as Attendance_Status
   ...
   FROM   
    ...
    WHERE   
        ...
        AND (ESD.inExamScheduleDetailId = @ExamID  OR @ExamID is NULL)
        AND (ESS.inSubjectID = @SubjectID  OR @SubjectID IS NULL)
