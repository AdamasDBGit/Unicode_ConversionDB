--EXEC [ERP_REPORTS].[usp_ERP_GetDueReport_HistoryNEW_Test] null,107,null,'2025-06-03','ALL'          
    --select * from T_Student_Class_Section where S_Student_ID='10-0163'    
CREATE PROCEDURE [ERP_REPORTS].[usp_ERP_GetDueReport_HistoryNEW_Test]                 
...
AS                 
  ...
  (                
    ...
    (A.I_Status=0 and C.I_Invoice_Detail_ID in(Select tc.I_Invoice_Detail_ID From T_Receipt_Component_Detail as tc                 
     ...
     and tc.I_Invoice_Detail_ID not in(Select I_Invoice_Detail_ID From T_Credit_Note_Invoice_Child_Detail)))                
                ...
                OUTER APPLY ( SELECT TOP 1                
                                        ...
                                        tsbd.I_Status ,                
                                        tsbd.I_Batch_ID                
                              FROM      dbo.T_Student_Batch_Details AS tsbd                                    WHERE     tsbd.I_Student_ID = a.I_Student_Detail_ID                
    ...
    AND convert(date,Dt_Installment_Date)<convert(date,@dtUptoDate)  
                AND E.I_Status = ISNULL(@StudentStatus, E.I_Status)                
                AND Isnull(C.Flag_IsAdvanceTax,'N') <> 'Y' and ISNULL(A.I_Status,0)<>0                
    ...
    --                            ELSE 'Own' END,                
    --            S_Brand_Name ,                
    --            S_Cost_Center ,                --            E.S_First_Name ,                
    --            E.S_Middle_Name ,                
    --            E.S_Last_Name,                
    --            CASE WHEN DATEDIFF(dd, '2017-07-01', Dt_Installment_Date) >= 0 THEN 1                
    --  ELSE 0                
    --END                
      --select * from #temp where S_Student_ID='25-0276'                  
  UPDATE T                
  ...
  SELECT  B.I_Invoice_Child_Header_ID ,                
                        ...
                        S_Cost_Center ,                
                        C.N_Amount_Due Due_Value ,                
                        Dt_Installment_Date ,                                        I_Installment_No ,                
                        A.I_Parent_Invoice_ID ,                
                        C.I_Invoice_Detail_ID ,                
                        CASE WHEN I_Parent_Invoice_ID IS NULL THEN NULL                
                             ELSE A.Dt_Crtd_On                
...
SELECT DISTINCT          
    ...
    T.S_Student_ID as StudentID,         
 sg.S_School_Group_Name AS School_Group,        
 tc.S_Class_Name AS Class,         
 st.S_Stream as Stream        
 ,ts.S_Section_Name as Section,        
     T.S_Mobile_No as MobileNo,          
    T.I_Roll_No as RollNo,          
    T.S_Student_Name as Student_Name,          
    T.S_Invoice_No as Invoice_No,          
    T.S_Receipt_No as Receipt_No,          
    convert(date,T.Dt_Invoice_Date) as Invoice_Date,          
    --T.I_Fee_Component_ID,          
    T.S_Component_Name as Fee_Component_Name,          
 --sg.S_School_Group_Name AS School_Group,          
 --tc.S_Class_Name AS Class,          
    --T.S_Batch_Name,          
    --T.S_Course_Name,          
   -- T.I_Center_ID,          
    --T.S_Center_Name,          
   -- T.TypeofCentre,          
T.S_Brand_Name as Brand,          
    --T.S_Cost_Center,          
    T.Due_Value,          
    T.Dt_Installment_Date as Installment_Date,          
    T.I_Installment_No as Installment_No,          
    --T.I_Parent_Invoice_ID,          
    --T.I_Invoice_Detail_ID,          
    T.Revised_Invoice_Date as Revised_Inv_Date,          
    T.Tax_Value,          
    T.Total_Value,          
    T.Amount_Paid,          
    T.Tax_Paid,          
    ...
    T.Total_Due,          
    --T.IsGSTImplemented,          
    CAST(DATEDIFF(d, T.Dt_Installment_Date, GETDATE()) AS INT) AS DueAge          
   -- NUll as instanceChain          
FROM #temp T          
    INNER JOIN dbo.fnGetCentreIdByBrandId(@iBrandID) FN1          
        ON T.I_Center_ID = FN1.CenterID          
 Left Join T_Student_Class_Section scs on scs.S_Student_ID=T.S_Student_ID          
 and scs.I_Status=1     
...
WHERE ISNULL(Total_Due, 0) > 0  --and t.S_Student_ID='25-0276'       
ORDER BY T.S_Student_ID              
