CREATE PROCEDURE [ERP_REPORTS].[usp_ERP_GetAdvanceformStudentReport_History] --exec [REPORT].[uspGetAdvanceformStudentReport_History] '93',111,'2017-07-31','ALL','2011-01-01'  
(  
...
AS   
  ...
  UPDATE #temp SET Effective_Advance = ISNULL(Effective_Advance, 0), Total_Paid = ISNULL(Total_Paid, 0), Tax_Paid = ISNULL(Tax_Paid, 0), Amount_Paid = ISNULL(Amount_Paid, 0)  
        ...
        SELECT
		T.S_Brand_Name BrandName,
		T.S_Center_Name Center,	
		T.S_Cost_Center CostCenter,
		T.S_Course_Name CourseName,
		T.S_Batch_Name BatchName,
		T.S_Student_ID StudentID,
		T.S_Student_Name StudentName,
		T.I_Roll_No RollNo,
		T.S_Mobile_No MobileNo	,	
		T.S_Invoice_No InvoiceNo,	
		T.Dt_Invoice_Date InvoiceDate,
		T.S_Component_Name ComponentName,
		T.S_Receipt_No ReceiptNo,
		T.Due_Value DueValue,
		T.Dt_Installment_Date "Instalment Date",
		T.I_Installment_No InstallmentNo,	
		T.Revised_Invoice_Date RevInvoiceDate,
		T.Tax_Value TaxValue,
