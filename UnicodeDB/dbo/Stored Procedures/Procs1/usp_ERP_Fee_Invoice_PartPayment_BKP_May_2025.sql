...
-- exec [usp_ERP_Fee_Invoice_PartPayment] 107,'16-0012'                      
                      
create PROCEDURE [dbo].[usp_ERP_Fee_Invoice_PartPayment_BKP_May_2025](@iBrandID INT,@sStudentID NVARCHAR(MAX))                      
AS                      
BEGIN                      
                      
      --DECLARE @iBrandID INT=107                      
      --DECLARE @sStudentID VARCHAR(MAX)='16-0012'                      
...
where i_brand_id=@iBrandID and I_School_Session_ID=@sessionID                  
  ...
  -- Select @FIYear                    
           ...
           IF OBJECT_ID('tempdb..#INVDET') IS NOT NULL    
           DROP TABLE #INVDET;             
...
EXEC usp_ERP_Fine_CalculateBased_On_Frequency @iBrandID,@sStudentID,@F_paymentdate          
   ...
   where I_Brand_ID=@iBrandID and Status_Type=2)                   
...
--Select 'Details'                    
select  DISTINCT S_Student_ID StudentID,I_Course_ID,S_Course_Name,DueType,I_Invoice_Header_ID as FeeScheduleID,I_Invoice_Detail_ID,S_Invoice_No InvoiceNo,FeeScheduleNo,I_Installment_No as InstallmentNo,Dt_Installment_Date,I_Sequence SequenceNo  
,I_FeeComponent_ID,        S_Component_Name ComponentName,N_Amount_Due,TotalTax,ISNULL(ReceiptCompAmount,0) as BaseAmountPaid,ISNULL(ReceiptCompTax,0) as TaxPaid,                      
...
Into #LoopGSTCal                    
from #INVDET_GST a                    
Left Join T_ERP_GST_Component_Mapping b on a.I_FeeComponent_ID=b.I_Fee_Component_ID and b.Is_Active=1                    
