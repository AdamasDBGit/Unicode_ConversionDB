CREATE PROCEDURE [dbo].[usp_ERP_Get_FeePlan_ComponentDetails]
...
,TEFS.Start_Period_Month as StartPeriodMonth
,TEFS.End_Period_Month as EndPeriodMonth
,TEFS.Payment_Schedule as PaymentSchedule
,TEFS.Is_Late_Fine_Applicable IsLateFineApplicable
,TEFS.R_I_FineRangeTagID FineRangeTagID,
TEFS.I_ERP_Fee_Category_Master_ID as FeeCategoryID
...
Where TBM.I_Brand_ID=ISNULL(@iBrand,TBM.I_Brand_ID)
   ...
   TERC.S_Fee_Component_Name AS FeeComponentName,
	...
	TEFPT.I_Interval AS PayInterval,
	TEFSIC.Is_During_Admission AS IsDuringAdmission,
	TEFSIC.I_Start_Period_Month as StartPeriodMonth,
	TEFSIC.I_End_Period_Month as EndPeriodMonth
   ...
   select DISTINCT
   EFSAM.I_School_Session_ID as AcademicSessionID,
   SASM.S_Label as AcademicSessionLabel,
   SASM.Dt_Session_Start_Date as AcademicSessionStart,
   SASM.Dt_Session_End_Date as AcademicSessionEnd,
   SASM.I_Current_Session as CurrentSession
   from T_ERP_Fee_Structure as EFS
   inner join
   T_ERP_Fee_Structure_AcademicSession_Map as EFSAM on EFS.I_Fee_Structure_ID=EFSAM.I_Fee_Structure_ID
   inner join
   T_School_Academic_Session_Master as SASM on EFSAM.I_School_Session_ID=SASM.I_School_Session_ID
   where EFSAM.Is_Active=1 and EFS.I_Fee_Structure_ID=@feeStructureID
