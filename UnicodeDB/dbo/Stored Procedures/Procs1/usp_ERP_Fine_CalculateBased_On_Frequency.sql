...
--EXEC   [usp_ERP_Fine_CalculateBased_On_Frequency]  107,'25-0438' ,'2025-09-17'                     
...
BEGIN                            
 ...
 End                  
 END    
 --select * from #InstallmentDatewiseFine
  ...
  T.N_FineAmount as ActualFineAmount
...
--order by sd.S_Student_ID            
 ...
 --and T_Tran.InstallmentDate=Convert(date,T.waiveoff_Installmentdate)            
  ...
  --Select * from #Final_InstallmentDatewiseFine    
   ...
   SELECT         
    ...
    ,ISNULL(fwh.FineDiscAmount,0) as FineDiscAmount      
    ,Case When FT.Is_waiveOff=0 Then       
    ISNULL((ft.ActualFineAmount -fwh.FineDiscAmount),0)       
    Else 0 end as CurrFineAmount      
    ,      
    FT.Is_waiveOff,        
    FT.Temp_Inv_No        
FROM #Final_InstallmentDatewiseFine FT        
LEFT JOIN (        
   Select IVP.I_Invoice_Header_ID,ICD.Dt_Installment_Date,ICD.S_Invoice_Number      
    ,MAX(ISNULL(ICD.Fine_waiveroff_Amt,0)) as FinewaieveOffAmount      
    ,MAX(ISNULL(ICD.FineDiscountAmount,0)) as FineDiscAmount  ,  
    MAX(ISNULL(ICD.Actual_FineAmount,0)) as ActualFineAmount  
    ...
    --where IVP.I_Invoice_Header_ID=370872       
    --and convert(date,ICD.Dt_Installment_Date)='2025-08-01'      
    Group By IVP.I_Invoice_Header_ID,ICD.Dt_Installment_Date,ICD.S_Invoice_Number       
) AS Fwh         
    ON fwh.I_Invoice_Header_ID = FT.InvoiceheaderID        
   AND fwh.Dt_Installment_Date = FT.waiveoff_Installmentdate    
   where ft.ActualFineAmount>0    
