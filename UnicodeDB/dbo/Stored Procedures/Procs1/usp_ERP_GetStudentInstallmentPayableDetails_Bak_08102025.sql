...
--exec usp_ERP_GetStudentInstallmentPayableDetails 107,'25-0249'

--EXEC [usp_ERP_GetStudentInstallmentPayableDetails] 107,'17-0221',null

create  PROCEDURE [dbo].[usp_ERP_GetStudentInstallmentPayableDetails_Bak_08102025]    
...
--select * from T_Student_Detail where S_Student_ID='25-0405'
    ...
    Declare @tenentID varchar(100)=
    (select top 1 tenant_ID from t_brand_master where I_Brand_ID=@iBrandID)
 ...
 Declare @FStartYear  VARCHAR(10) ,@FEndYear Varchar(10),@sessionID int  ,@sessionStartdate date 
 SET @sessionID=(  
...
Print  @sessionID  
select Top 1  @FStartYear=YEAR(Dt_Session_Start_Date)
,@FEndYear=YEAR(Dt_Session_End_Date ),@sessionStartdate=Convert(Date,Dt_Session_Start_Date)
...
where i_brand_id=@iBrandID and I_School_Session_ID=@sessionID  
  ...
  ,LinkedAccountID nvarchar(100)
   ...
   --,Status  
      ...
      TSBM1.S_Batch_Name ,                        
                        ...
                        LTRIM(RTRIM(
				TSD.S_First_Name
				+ CASE WHEN TSD.S_Middle_Name IS NULL OR TSD.S_Middle_Name = '' 
					   THEN '' ELSE ' ' + TSD.S_Middle_Name END
				+ CASE WHEN TSD.S_Last_Name IS NULL OR TSD.S_Last_Name = '' 
					   THEN '' ELSE ' ' + TSD.S_Last_Name END
			)) AS StudentName ,                        
      ...
      TICD.I_Fee_Component_ID,                        
                        ...
                        TICD.N_Amount_Due,                        
      CASE
	  WHEN TICD.Dt_Installment_Date < @sessionStartdate 
     THEN 'Previous'
 ...
 FROM    dbo.T_Invoice_Parent TIP                        
                        ...
                        LEFT JOIN T_ERP_BankMaster TEB ON TEB.ID=TFCM.I_Bank_ID and teb.inBrandID=@iBrandID
...
--Select @TotalFinePaid  
 ...
 , LinkedAccountID
  ...
  group by Dt_Installment_Date  ,LinkedAccountID
...
Left Join T_Status_Master TSM ON TSM.I_Status_Value=f.I_FeeComponent_ID 
and TSM.I_Status_Value=@finecomponentID
Left Join T_ERP_BankMaster EBMS on EBMS.ID=TSM.BankID
where sf.FineAmount >0  --AND LinkedAccountID IS NOT  NULL    
--and LinkedAccountID=@tenentID
...
Update #PayableHeader1 
set Adhoc_TotalAmount=ISNULL(@TotalAdhocamt,0)+ISNULL(@TotalAdhocPaid,0)
--et Adhoc_TotalAmount=@TotalFineHeader1+ISNULL(@TotalAdhocamt,0)+ISNULL(@TotalAdhocPaid,0)
...
Else NULL End
as Fee_Remarks,
ISNULL((@TotalFineHeader1+@TotalFinePaid),0) as TotalFinePayable
...
ELSE
	CASE WHEN (select count(*) from T_Invoice_Child_Detail 
    where p4.InvoiceNo=S_Invoice_Number 
    and ISNULL(is_Freezed,0)=1) > 0  
	THEN 1 ELSE 0 END
END
 as Freezed 
 ,ISNULL(ICCD.N_Disc_Amount,0) as L3DiscAmt 
 ...
 EBM.LinkedAccountId AS LinkedAccountId
...
--and ICCD.I_Fee_Component_ID=p4.I_FeeComponent_ID 
Left JOin (
select distinct ICH.I_Invoice_Header_ID,ICD.I_Invoice_Detail_ID
,ICD.I_Fee_Component_ID,ICD.N_Disc_Amount,ICD.N_Gross_Amount
,ICD.N_Tax_Disc_Amount,ICD.N_Fix_Discount,ICD.N_Perc_Discount,
convert(date,ICd.Dt_Installment_Date) as Dt_Installment_Date,
ICD.S_Invoice_Number,ICD.is_Freezed,ICD.N_Amount_Due
...
and ICH.I_Course_FeePlan_ID IS NOT NULL
) ICCD ON ICCD.I_Invoice_Header_ID=p4.FeeScheduleID
and ICCD.Dt_Installment_Date=convert(date,p4.Dt_Installment_Date)
and ICCD.S_Invoice_Number=p4.InvoiceNo
and ICCD.I_Fee_Component_ID=p4.I_FeeComponent_ID 
and ICCD.I_Invoice_Detail_ID=p4.I_Invoice_Detail_ID
--Left Join T_Invoice_Child_Header ICCH 
--on ICCH.I_Invoice_Child_Header_ID=ICCD.I_Invoice_Child_Header_ID          
--and ICCH.I_Invoice_Header_ID=p4.FeeScheduleID    
Left Join T_Fee_Component_Master FCM ON FCM.I_Fee_Component_ID=p4.FeeComponentID
Left Join T_ERP_BankMaster EBM ON EBM.ID=FCM.I_Bank_ID
