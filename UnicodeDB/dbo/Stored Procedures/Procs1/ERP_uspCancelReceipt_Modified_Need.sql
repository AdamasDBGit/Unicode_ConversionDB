--exec [ERP_uspCancelReceipt] 1378310,null,'dba',0
CREATE PROCEDURE [dbo].[ERP_uspCancelReceipt_Modified_Need]      -- [dbo].[uspCancelReceipt] 3965,'',null  
    (  
      @iReceiptHeaderId INT ,  
      @sCancellationReason NVARCHAR(MAX)=null ,  
...
BEGIN
		...
		(  
		 ...
		 BEGIN
			...
			EXEC dbo.ERP_uspInsertInvoiceDetailsForOnAccountReceipt @iReceiptHeaderId
			
		 END
        
        EXEC ERP_uspInsertCreditNoteForCancelReceipt @iReceiptHeaderId  
		...
		select 1 StatusFlag,'Receipt has been cancelled Successfully' Message
		...
		begin
			...
			select 0 StatusFlag,'Cancellation of the receipt is not allowed after the parent invoice has been marked as inactive.' Message
