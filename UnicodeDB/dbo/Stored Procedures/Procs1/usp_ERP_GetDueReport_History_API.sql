...
INNER JOIN #temphistory T ON RCD.I_Invoice_Detail_ID = T.I_Invoice_Detail_ID              
 ...
 Left Join T_Stream st on st.I_Stream_ID=scs.I_Stream_ID and st.I_brand_id=@iBrandID  
  ...
  AND sg.S_School_Group_Name=@StrSchool_Group          
    ...
    --(
 --   SELECT DISTINCT
 --   T.I_Student_Detail_ID , T.S_Mobile_No ,T.S_Student_ID ,  
 ...
 -- ORDER BY S_Center_Name, S_Batch_Name, I_Roll_NO
    ...
    T.S_Student_Name,
    SUM(T.Due_Value) AS Due_Value,
    MAX(T.Dt_Installment_Date) AS Dt_Installment_Date,                 
