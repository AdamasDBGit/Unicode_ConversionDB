...
BEGIN      
    ...
    SELECT 
        ...
        SD.I_Enquiry_Regn_ID AS EnquiryNo,
        ...
        CASE WHEN RH.I_Invoice_Header_ID IS NOT NULL THEN 0 ELSE 1 END AS isAdhoc,
        ...
        CASE WHEN RH.I_Invoice_Header_ID IS NOT NULL THEN 'Scheduled' ELSE 'Adhoc' END AS ReceiptStatusType,
        ROW_NUMBER() OVER (PARTITION BY RH.I_Receipt_Header_ID, TM.I_ERP_TransactionNo ORDER BY TM.Dt_TransactionDate DESC) AS rn
    ...
    WHERE SD.S_Student_ID = ISNULL(@sStudentID, SD.S_Student_ID)
      ...
      AND (CASE WHEN RH.I_Invoice_Header_ID IS NOT NULL THEN 0 ELSE 1 END = ISNULL(@bIsAdhoc, CASE WHEN RH.I_Invoice_Header_ID IS NOT NULL THEN 0 ELSE 1 END))
)
...
WHERE rn = 1;
    ...
    --    CASE WHEN RH.I_Invoice_Header_ID IS NOT NULL THEN 0 ELSE 1 END,      
    ...
    --    CASE WHEN RH.I_Invoice_Header_ID IS NOT NULL THEN 'Scheduled' ELSE 'Adhoc' END      
    --FROM T_Receipt_Header RH WITH (NOLOCK)      
    --INNER JOIN dbo.T_PaymentMode_Master PMM WITH (NOLOCK) ON RH.I_PaymentMode_ID = PMM.I_PaymentMode_ID      
    --INNER JOIN dbo.T_Status_Master RT WITH (NOLOCK) ON RT.S_Status_Type = 'ReceiptType' AND RH.I_Receipt_Type = RT.I_Status_Value      
    --INNER JOIN dbo.T_Centre_Master CEM WITH (NOLOCK) ON RH.I_Centre_Id = CEM.I_Centre_Id      
