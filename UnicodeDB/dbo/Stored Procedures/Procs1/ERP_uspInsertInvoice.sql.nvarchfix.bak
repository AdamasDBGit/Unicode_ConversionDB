...
AS                       
  ...
  DECLARE @BatchID INT=0                      
     ...
     N_Amount_Payable NUMERIC(18, 2) ,                      
              ...
              I_Sequence INT,   
     N_Actual_Amount Numeric(18,2),  
     N_DISC_Amount Numeric(18,2),  
	 N_Tax_Disc_Amount Numeric(18,2),
	 N_DISC_IGST_Tax_Amount Numeric(18,2),
              ID INT IDENTITY(1,1)                      
            )                       
...
-- insert value for the invoice child                                 
        INSERT  INTO #tempInvoiceChild                      
            ...
            -------Set Currency ------------              
   Declare @CurrencyID int ,@I_Course_FeePlan_ID int ,@NewFeeStructureID int--,@SessionID int           
   ,@studentID int          
   SET @I_Course_FeePlan_ID=(Select top 1 I_Course_FeePlan_ID from #tempInvoiceChild)              
   SET @CurrencyID=(select top 1 I_Currency_ID from T_Course_Fee_Plan with(Nolock)            
   where I_Course_Fee_Plan_ID=@I_Course_FeePlan_ID          
   )              
 SET @studentID=(Select top 1 I_Student_Detail_ID from #tempInvoice)         
 print @studentID      
 --SET @SessionID=(          
 --select top 1 I_School_Session_ID from T_Student_Class_Section where I_Student_Detail_ID=@studentID           
 --and I_Status=1          
 --)        
 ------Need to revise code in future-------------  
 IF @SessionID IS NULL 
 BEGIN

 SET @SessionID=(      
 select I_School_Session_ID from T_School_Academic_Session_Master where I_Brand_ID=@iBrandID      
 ...
 IF @iParentInvoiceID IS NOT NULL          
 BEGIN          
 SET @SessionID=(select I_School_Session_ID from T_invoice_Parent where I_Invoice_Header_ID=@iParentInvoiceID )          
 END          
          
 ------------------------------          
          
---------------------------------Set Fine Implement------------------------------------------------            
Declare @Is_Fine_Applicable bit, @FineTagId int             
Set @NewFeeStructureID=(            
   select top 1 I_New_I_Fee_Structure_ID from T_Course_Fee_Plan with(Nolock)            
   where I_Course_Fee_Plan_ID=@I_Course_FeePlan_ID            
   )           
     Set @Is_Fine_Applicable=(            
  Select top 1 isnull(Is_Late_Fine_Applicable ,0)            
  from T_ERP_Fee_Structure with(Nolock) where I_Fee_Structure_ID=@NewFeeStructureID            
  )            
  Set @FineTagID=(            
...
-- insert values for the invoice child details                         
      ...
      T.c.value('@N_Actual_Amount', 'numeric(18, 2)') ,  
      T.c.value('@N_DISC_Amount', 'numeric(18, 2)')  , 
	  T.c.value('@N_DISC_Tax_Amount', 'numeric(18, 2)') ,
	  T.c.value('@N_DISC_IGST_Tax_Amount', 'numeric(18, 2)') 
...
--DEFINATION OF THE VARIABLES OF THE INVOICE CHILD DETAILS                                
  ...
  DEclare @icdN_Actual_Amount Numeric(18,2)  
  Declare @icdN_DISC_Amount Numeric(18,2)  
  Declare @icdN_Tax_Disc_Amount Numeric(18,2)
  Declare @icdN_DISC_IGST_Tax_Amount Numeric(18,2)                           
...
-- DEFINATION OF THE VARIABLE TO INSERT TAX DETAILS                                
 ...
 @ipDt_Crtd_On                                
   ...
   ( I_Student_Detail_ID ,                      
                          ...
                          N_Invoice_Amount ,                      
                          N_Discount_Amount ,                      
                          I_Coupon_Discount ,                              N_Tax_Amount ,                      
                          Dt_Invoice_Date ,                      
                          I_Status ,                      
                          I_Discount_Scheme_ID ,                      
                          I_Discount_Applied_At ,                      
                ...
                VALUES  ( @ipI_Student_Detail_ID ,                      
                          ...
                          @ipI_Coupon_Discount ,                      
                          @ipN_Tax_Amount ,                      
                          DATEADD(minute,-1,@ipDt_Invoice_Date) ,--susmita:decrease invoice date -1 min                      
                          @ipI_Status ,                      
                          @ipI_Discount_Scheme_ID ,                      
                          @ipI_Discount_Applied_At ,                      
                          @ipS_Crtd_By ,                      
                          --DATEADD(minute,-1,@ipDt_Crtd_On),--susmita:decrease invoice date -1 min 
                          GETDATE(),
                          @iParentInvoiceID                      
                        )                                
...
SET @iInvoiceID = SCOPE_IDENTITY()                                
                ...
                Update T_Invoice_Parent set Is_NewGSTEnvironment=1 where I_Invoice_Header_ID=@iInvoiceID                  
    ...
    @icI_Discount_Applied_At                                  
      ...
      I_Status ,                      
                                ...
                                VALUES  ( @iInvoiceChildID ,                      
                                          ...
                                          @ipI_Status ,                      
                                          @ipS_Crtd_By ,                      
                                          DATEADD(minute,-1,@ipDt_Crtd_On) --susmita: invoice creation date decrease by 1 min                      
                                        )                      
   ...
   @icdN_Actual_Amount,  
   @icdN_DISC_Amount  ,
   @icdN_Tax_Disc_Amount,
   @icdN_DISC_IGST_Tax_Amount
...
-- Update Course wise fees paid for the invoice                                
        ...
        WHILE @@FETCH_STATUS = 0                       
            BEGIN                                 
                ...
                WHERE   I_Invoice_Child_Header_ID = @iInvoiceChildHeaderID 
                --------Update Gross Amount in Child Header------
                Declare @ChildGrossAmt Numeric(12,2)
                SELECT  @ChildGrossAmt = SUM(N_Gross_Amount)                      
                FROM    dbo.T_Invoice_Child_Detail                      
                WHERE   I_Invoice_Child_Header_ID = @iInvoiceChildHeaderID

                UPDATE  dbo.T_Invoice_Child_Header                      
                SET     N_Gross_Amount = @ChildGrossAmt                       
  ...
  END                      
    ...
    ------Updating GST Amount columnwise in New System---------------                  
    UPDATE a set                   
   a.N_CGST= Case When (I_Tax_ID=7 Or I_Tax_ID=8 )  Then d.N_Tax_Value  else Null end ,                  
   a.N_SGST= Case When (I_Tax_ID=8 Or I_Tax_ID=7 ) Then d.N_Tax_Value  else Null end,                  
   a.N_IGST=Case When I_Tax_ID=9 Then d.N_Tax_Value  else Null end                   
   --Into #tempGST_Update                  
   from   T_Invoice_Child_Detail a                  
   Inner Join  T_Invoice_Child_Header b on a.I_Invoice_Child_Header_ID=b.I_Invoice_Child_Header_ID                  
   Inner Join T_Invoice_Parent c on c.I_Invoice_Header_ID=b.I_Invoice_Header_ID                  
  Inner Join T_Invoice_Detail_Tax d on d.I_Invoice_Detail_ID=a.I_Invoice_Detail_ID                  
   where c.I_Invoice_Header_ID=@I_Invoice_Header_ID                  
     -------Update Currency--------------             
   Update   T_Invoice_Parent set I_Currency_Id=@CurrencyID           
   where I_Invoice_Header_ID=@I_Invoice_Header_ID              
   ---------------------Fine Update------------------------------            
   Update   T_Invoice_Parent set I_FineTagID=@FineTagId,is_Fine_Applicable=@Is_Fine_Applicable             
   where I_Invoice_Header_ID=@I_Invoice_Header_ID            
  --SELECT  @iInvoiceID AS I_Invoice_ID               
  ----------update Session ID-------          
     Update   T_Invoice_Parent set I_School_Session_ID=@SessionID           
   where I_Invoice_Header_ID=@I_Invoice_Header_ID            
   ------Implement  Discount Template ID and Gross Amount----
   
   DECLARE @DiscountSchemeID INT;
   SET @DiscountSchemeID=(
   select I_Discount_Scheme_ID  from #tempInvoice
   )
    IF   @DiscountSchemeID IS NOT NULL
	Begin
    Update   T_Invoice_Parent set I_DiscountType_ID =1            
    where I_Invoice_Header_ID=@I_Invoice_Header_ID   
	End
	-------------------------------------------------------
	
DECLARE @TotalActualGrossAmount NUMERIC(18,2);
SET @TotalActualGrossAmount=(
select SUM(ISNULL(N_Actual_Amount,0)) from #tempInvoiceChildDetail
)

    Update T_Invoice_Parent set N_Gross_Amount =@TotalActualGrossAmount         
    where I_Invoice_Header_ID=@I_Invoice_Header_ID 
          
   IF @iParentInvoiceID IS NOT NULL          
   Begin          
      Declare @ClassID int,@streamID int,@ClassGroupID int          
          
          
   select          
@ClassID=SGC.I_Class_ID,@streamID=SCS.I_Stream_ID          
,@ClassGroupID=SGC.I_School_Group_Class_ID          
...
TIP.I_Invoice_Header_ID=@iParentInvoiceID and SCS.I_Student_Detail_ID=TIP.I_Student_Detail_ID          
...
update T_Student_Class_Section set I_Status=1 where          
I_Status=0 and I_School_Session_ID=@SessionID and I_School_Group_Class_ID=@ClassGroupID          
and I_Student_Detail_ID= @StudentDetailID           
---------------Insert Route Pickup Map--------------        
--Declare @IstudentID int      
--SET @IstudentID=(select I_Student_Detail_ID from T_Invoice_Parent with(nolock)       
--where I_Invoice_Header_ID=@I_Invoice_Header_ID)      
       
          
          
--Select * from T_Student_Transport_History           
   End          
...
Declare @RouteID int,@PickupID int          
    --print @EnquiryID      
 Print 'ttt'      
Select top 1 @PickupID=cmd.I_ExtracomponentRef_ID ,@RouteID=cmd.I_ExtracomponentRef2_ID           
...
where cm.R_I_Enquiry_Regn_ID=@EnquiryID and cm.R_I_School_Session_ID=@SessionID         
and cmd.I_ExtracomponentRef_ID is not null and cmd.I_ExtracomponentRef2_ID  is not null      
and cmd.R_I_Fee_Structure_ID=0 and (cmd.Is_Moved IS NULL OR cmd.Is_Moved =0)      
--select @PickupID,@RouteID      
-----          
IF @PickupID is not null and @RouteID is not null       
Begin      
Insert Into T_Student_Transport_History(          
I_Student_Detail_ID          
,I_PickupPoint_ID          
,I_Route_ID          
...
Select @studentID,@PickupID,@RouteID,Null,52,GETDATE(),Null,NULL          
--where not Exists(          
--select 1 from T_Student_Transport_History trg where trg.I_Student_Detail_ID=@studentID          
--and trg.I_PickupPoint_ID=@PickupID and trg.I_Route_ID=@RouteID          
--)         
update T_Student_Detail set I_Route_ID=@RouteID, I_Transport_ID=@PickupID  from T_Student_Detail         
where I_Student_Detail_ID=@studentID                     
   End                             
