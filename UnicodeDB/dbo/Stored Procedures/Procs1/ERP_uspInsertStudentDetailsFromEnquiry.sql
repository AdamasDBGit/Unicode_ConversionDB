  ...
  --EXEC [ERP_uspInsertStudentDetailsFromEnquiry] 251223,'dba','2025-07-09',null,'221',11,null,107
...
SELECT TOP 1              
    @DesiredSectionID = ISNULL(I_Section_ID,1)              
FROM              
    T_Section              
WHERE       is_Default=1        
...
Begin              
   Insert Into T_Student_Class_Section(              
    ...
    --WHERE I_Section_ID = @DesiredSectionID;              
      -----------------Map Student Transport Details -------      
   Declare @pickupid int ,@RouteID int    
   Select top 1 @pickupid=scmd.I_ExtracomponentRef_ID     
,@RouteID=scmd.I_ExtracomponentRef2_ID     
from T_ERP_Stud_Fee_Struct_Comp_Mapping scm    
Inner Join T_ERP_Stud_Fee_Struct_Comp_Mapping_Details scmd     
ON scm.I_Stud_Fee_Struct_CompMap_ID=scmd.R_I_Stud_Fee_Struct_CompMap_ID    
where scmd.R_I_Fee_Structure_ID=0 and scm.R_I_Enquiry_Regn_ID=@iEnquiryRegnID    
and scmd.I_ExtracomponentRef_Type=1 and scmd.Is_Active=1    
If Not exists (    
select 1 from T_Student_Transport_History where I_Student_Detail_ID=@iStudentDetailId    
and I_Route_ID=@RouteID and I_PickupPoint_ID=@pickupid    
)    
Begin    
...
-- ---------------------------------------------------- --------------------------                    
 ...
 --Error occurred:                          
        ...
        DECLARE @ErrMsg NVARCHAR(4000) ,                      
                @ErrSeverity INT,@errline varchar(max)                        
        SELECT  @ErrMsg = ERROR_MESSAGE() ,                      
                @ErrSeverity = ERROR_SEVERITY() ,  @errline=ERROR_LINE()                     
            --select    @errline         
