...
-- =============================================
CREATE   PROCEDURE [dbo].[usp_ERP_Make_Transaction_Success_BKP_Sept_2025]
...
SELECT 
	...
	CONVERT(datetime, T.c.value('@InstallmentDate', 'nvarchar(50)'), 126) as InstallmentDate,
	T.c.value('@PaymentScheduleID', 'int') AS PaymentScheduleID
FROM 
...
and TID.I_Invoice_Header_ID=@iFeeSchedule
and ISNULL(TID.CanBeProcessed,'false')='true' and ISNULL(TID.IsCompleted,'false')='false'
and ADT.PaymentScheduleID IS NULL
...
#AdhocDetailsTable as ADT on CONVERT(DATE,ADT.InstallmentDate)=CONVERT(DATE,TID.Dt_Installment_Date)
inner join
T_ERP_AdhocPaymentScheduleStudentDetail as APSD on ADT.PaymentScheduleID=APSD.inAdhocPaymentScheduleStudentDetailID
and ADT.StatusValue=TID.StatusValue
where ETM.I_ERP_TransactionNo=@sTransactionNo
and ISNULL(TID.CanBeProcessed,'false')='true' and ISNULL(TID.IsCompleted,'false')='false'
and ADT.PaymentScheduleID IS NOT NULL

-- Add row numbers for iteration
SELECT ROW_NUMBER() OVER (ORDER BY (SELECT NULL)) AS RowNum,
       PaymentScheduleID
INTO #TempAdhoc
FROM #AdhocDetailsTable;

DECLARE @Max INT = (SELECT MAX(RowNum) FROM #TempAdhoc);
DECLARE @Counter INT = 1;
DECLARE @PaymentScheduleID INT;

WHILE @Counter <= @Max
BEGIN
    SELECT @PaymentScheduleID = PaymentScheduleID
    FROM #TempAdhoc
    WHERE RowNum = @Counter;

    EXEC [dbo].[usp_ERP_UpdateAdHocPaymentStatus] @PaymentScheduleID, 1, @OnAccountReceiptHeader;
