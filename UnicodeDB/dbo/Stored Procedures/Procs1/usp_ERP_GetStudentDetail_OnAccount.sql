...
--exec [usp_ERP_GetStudentDetail_OnAccount] 237228,null,107
--select * from T_Class
-- =============================================
...
BEGIN
    ...
    ISNULL(
        CASE 
            WHEN TSGS.I_Class_ID IS NOT NULL THEN TC.S_Class_Name
            ELSE (SELECT TC2.S_Class_Name 
                  FROM T_Class TC2 
                  WHERE TC2.I_Class_ID = ERD.I_Class_ID) 
        END, 'NA') AS ClassName,
    ERD.S_Mobile_No AS MobileNo,
    ERD.I_Class_ID AS PreviousClass,
    TSGS.I_Class_ID AS CurrentClass
FROM 
...
LEFT JOIN 
    ...
    ON TSCS.I_Student_Detail_ID = TSD.I_Student_Detail_ID AND TSCS.I_Status = 1
LEFT JOIN 
