...
CREATE   PROCEDURE [dbo].[usp_ERP_InitiateTransaction_Using_for_Adhoc_T_BKP_Sept_2025]
...
BEGIN
   ...
   BEGIN TRANSACTION;	
	...
	IF @iPaymentGatewayBrandID IS NULL OR @iPaymentGatewayBrandID = 0
	begin

	SELECT top 1
		@iPaymentGatewayBrandID=EBPM.I_ERP_Brand_PaymentGateway_Map_id 
		,@iSmsPaymentMode=EBPM.I_Payment_Mode
		from 
...
--set @XmlData='<Root><RowFeeSheduleDtl FeeScheduleID="0"><TblDueOnAccountDtl><RowDueOnAccountDtl Amount="100" InvoiceNo="TEMP/000260" InstallmentDate="27-05-2025 00:00:00" StatusValue="147" PaymentScheduleID="260"><ReceiptTax /></RowDueOnAccountDtl><RowDueOnAccountDtl Amount="90" InvoiceNo="TEMP/000259" InstallmentDate="27-05-2025 00:00:00" StatusValue="219" PaymentScheduleID="259"><ReceiptTax><TaxDetails TaxID="0" TaxPaid="4.5" /><TaxDetails TaxID="0" TaxPaid="4.5" /></ReceiptTax></RowDueOnAccountDtl></TblDueOnAccountDtl></RowFeeSheduleDtl></Root>';
    ...
    SELECT 
		...
		CONVERT(datetime, AdhocDetail.value('@InstallmentDate', 'nvarchar(100)'), 126) as InstallmentDate,
...
CROSS APPLY AdhocDetail.nodes('ReceiptTax/TaxDetails') AS TaxDetails(TaxDetails);
	...
	--select @iTransactionMasterID,IT.FeeScheduleID,IT.StatusValue,IT.InvoiceNo,IT.InstallmentDate,IT.Amount,1,@sStudentID ,
	--ISNULL(ATT.TaxPaid,0),IT.PaymentScheduleID
	--from #AdhocDetailsTable as IT
	--left join
	--#OnAccountTaxTable as ATT on IT.InvoiceNo=ATT.InvoiceNo
	--group by IT.FeeScheduleID,IT.InstallmentDate,IT.InvoiceNo,IT.StatusValue,IT.PaymentScheduleID
	...
	select @iTransactionMasterID,IT.FeeScheduleID,IT.StatusValue,IT.InvoiceNo,IT.InstallmentDate,IT.Amount,1,@sStudentID ,
	sum(ISNULL(ATT.TaxPaid,0)),IT.PaymentScheduleID
	from #AdhocDetailsTable as IT
	left join
	#OnAccountTaxTable as ATT on IT.InvoiceNo=ATT.InvoiceNo
	group by IT.FeeScheduleID,IT.InstallmentDate,IT.InvoiceNo,IT.StatusValue,IT.PaymentScheduleID,IT.Amount
	...
	update EAPSSD set EAPSSD.IsFreeze='true'
	from #AdhocDetailsTable as SDT
	inner join
	T_ERP_AdhocPaymentScheduleStudentDetail as EAPSSD on EAPSSD.inAdhocPaymentScheduleStudentDetailID=SDT.PaymentScheduleID
	...
	@sTransactionStatus,--@currentStatus nvarchar(max)=NULL,
