...
BEGIN   
    ...
    INSERT INTO #ICHNW(I_Invoice_Detail_ID,I_Fee_Component_ID,I_Invoice_Child_Header_ID,I_Invoice_Header_ID,I_Installment_No,
                Dt_Installment_Date,N_Amount_Due,N_Amount_Adv,S_Invoice_Number,Flag_IsAdvanceTax) -- New 
    ...
    ORDER BY B.I_Installment_No ASC -- Those Invoices will be consider whose invoicedetail not cleared in receipts
    ...
    GROUP BY IC.I_Invoice_Detail_ID, IC.I_Invoice_Header_ID --- Get existing credited invoice Details
    ...
    GROUP BY CNT.I_Tax_ID, CNT.I_Invoice_Detail_ID --- Get existing credited invoice tax Details


	IF exists(select * from T_Invoice_Parent where I_Invoice_Header_ID=@iInvoiceId 
	and ISNULL(Is_NewGSTEnvironment,'false')='false'
	)

	BEGIN

    INSERT INTO #ITAX(I_Tax_ID,I_Invoice_Detail_ID,N_Tax_Value) -- Get Invoice Tax Details
	...
	DECLARE @CGSTID INT=0
	DECLARE @SGSTID INT=0
	DECLARE @IGSTID INT=0

	select @CGSTID=I_Tax_ID from T_Tax_Master where S_Tax_Code='CGST'
	select @SGSTID=I_Tax_ID from T_Tax_Master where S_Tax_Code='SGST'
	select @IGSTID=I_Tax_ID from T_Tax_Master where S_Tax_Code='IGST'

	INSERT INTO #NewEnviromentTAXID
	(
	I_Tax_ID,
	S_Tax_Name,
	I_Invoice_Detail_ID,
	...
	WHERE ICH.I_Invoice_Header_ID = @iInvoiceId
    AND ISNULL(ICD.Flag_IsAdvanceTax,'N') <> 'Y'
    AND ISNULL(ICD.N_IGST,0) > 0



	INSERT INTO #ITAX(I_Tax_ID,I_Invoice_Detail_ID,N_Tax_Value) -- Get Invoice Tax Details
    SELECT I_Tax_ID,I_Invoice_Detail_ID,SUM(ISNULL(N_Tax_Value,0))
    FROM
	#NewEnviromentTAXID
    GROUP BY I_Tax_ID, I_Invoice_Detail_ID
    order by I_Invoice_Detail_ID
    ...
    INSERT INTO #RTAX(I_Tax_ID,I_Invoice_Detail_ID,N_Tax_Value) --- --- Get Receipt tax Details
        ...
        BEGIN
			...
			IF exists(select * from T_Invoice_Parent where I_Invoice_Header_ID=@iInvoiceId 
						...
						ELSE
							...
							INSERT INTO T_Credit_Note_Invoice_Child_Detail_Tax(I_Credit_Note_Invoice_Child_Detail_ID,I_Tax_ID,I_Invoice_Detail_ID,N_Tax_Value)
								SELECT @I_Credit_Note_Invoice_Child_Detail_ID, IDT.I_Tax_ID, IDT.I_Invoice_Detail_ID, SUM(IDT.N_Tax_Value) - SUM(ISNULL(ICT.N_Tax_Value,0))
								FROM #NewEnviromentTAXID IDT
								LEFT JOIN #ICTNX ICT  ON (IDT.I_Tax_ID = ICT.I_Tax_ID AND IDT.I_Invoice_Detail_ID = ICT.I_Invoice_Detail_ID)
								WHERE IDT.I_Invoice_Detail_ID = @I_Invoice_Detail_ID
								GROUP BY IDT.I_Tax_ID, IDT.I_Invoice_Detail_ID
								HAVING (SUM(IDT.N_Tax_Value) - SUM(ISNULL(ICT.N_Tax_Value,0))) > 0
			 
							END
           
      --------     IF(@Flag_IsAdvanceTax = 'Y') -- stop for now : 2024-08-02
      --------       BEGIN --
