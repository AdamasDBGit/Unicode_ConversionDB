CREATE PROCEDURE [dbo].[USP_GetFirebaseTokensByStudentIds_Bak_20042025]      
...
TAD.enquiry_no,      
   ...
   SD.I_Parent_Master_ID as inParentMasterID  
        ...
        LEFT JOIN T_School_Group SG        ON SG.I_School_Group_ID = TSGC.I_School_Group_ID AND SG.I_Brand_Id = @inBrandID      
        LEFT JOIN         
 ...
 ELSE IF @inRecipientId = 1      
  ...
  WHERE U.I_Status = 1      
   ...
   U.S_Mobile AS username_mobile,      
   U.S_FireBase_Token AS firebase_token,       UP.Dt_DOB AS birthday_year,      
   '' AS work_experience_years,      
   UP.Dt_DOJ AS join_date      
