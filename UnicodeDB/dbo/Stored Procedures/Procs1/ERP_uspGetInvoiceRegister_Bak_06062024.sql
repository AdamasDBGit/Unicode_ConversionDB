...
CREATE PROCEDURE [dbo].[ERP_uspGetInvoiceRegister_Bak_06062024] --ERP_uspGetInvoiceRegister 53,107,NULL,NULL,'2013-04-03','','','','04-0002',''         
    (          
...
AS           
 ...
 --AND DATEDIFF(dd, @dtDateFrom,IP.Dt_Invoice_Date) >= 0           
                        ...
                        --AND   
      --ISNULL(SD.S_First_Name, '') LIKE ISNULL(@sFName, '')          
      --                  + '%'          
      --                  AND ISNULL(SD.S_Middle_Name, '') LIKE ISNULL(@sMName, '')          
      --                  + '%'          
      --                  AND ISNULL(SD.S_Last_Name, '') LIKE ISNULL(@sLName, '')          
      --                  + '%'     
  -- CONCAT(Sd.S_First_Name, ' ', ISNULL(Sd.S_Middle_Name + ' ', ''), Sd.S_Last_Name) LIKE ISNULL(@FullName, '')  
