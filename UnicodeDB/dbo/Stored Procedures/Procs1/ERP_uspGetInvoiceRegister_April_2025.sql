...
create PROCEDURE [dbo].[ERP_uspGetInvoiceRegister_April_2025] --ERP_uspGetInvoiceRegister 53,107,NULL,NULL,'2013-04-03','','','','04-0002',''           
    (            
...
AS             
    ...
    Where I_Brand_ID=@iSelectedBrandId    
        ...
        INSERT  INTO @InvoiceDetail            
                SELECT  DISTINCT IP.S_Invoice_No,            
                        IP.I_Invoice_Header_ID,            
                        IP.N_Invoice_Amount,            
                        IP.Dt_Invoice_Date,            
                        IP.I_Student_Detail_ID,            
                        SD.S_Student_ID,            
                        Ltrim(Rtrim((SD.S_First_Name))),            
                        Ltrim(Rtrim((SD.S_Middle_Name))),            
                        Ltrim(Rtrim((SD.S_Last_Name))), 
						IP.I_Status,
						...
						,ISNULL(IP.S_Narration,ISNULL(partialremarks.Merged_Remarks,'')) as Remarks
						,ISNULL(parent.S_Invoice_No,0) as ParentInvoiceNo
                FROM    dbo.T_Invoice_Parent IP            
						...
						left join
						T_Invoice_Parent as parent on IP.I_Parent_Invoice_ID=parent.I_Invoice_Header_ID
						LEFT JOIN (
        SELECT 
    Invoice_Header_ID,
...
GROUP BY Invoice_Header_ID, CAST(CreatedOn AS DATE) 
    ) AS partialremarks  on partialremarks.Invoice_Header_ID=IP.I_Invoice_Header_ID
    ...
    IP.I_Centre_Id IN ( SELECT  I_Center_ID            
      ...
      AND DATEDIFF(dd, @dtDateFrom,CONVERT(DATE,IP.Dt_Invoice_Date)) >= 0             
                        ...
                        AND     
      CONCAT(LTRIM(RTRIM((Sd.S_First_Name))), ' ', ISNULL(LTRIM(RTRIM((Sd.S_Middle_Name))) + ' ', ''), LTRIM(RTRIM((Sd.S_Last_Name)))) LIKE ISNULL(@FullName, CONCAT(LTRIM(RTRIM((Sd.S_First_Name))), ' ', ISNULL(LTRIM(RTRIM((Sd.S_Middle_Name))) + ' ', ''), LTRIM(RTRIM((Sd.S_Last_Name)))))+'%'    
 ...
 AND (@InvoiceStatus IS NULL or (IP.I_Status=@InvoiceStatus))
