...
-- =============================================
CREATE   PROCEDURE [dbo].[usp_ERP_Get_Pagination_Wise_Transaction_History]
    ...
    @PageSize INT = 100,   -- Default page size
	@isOnline bit=NULL
AS
BEGIN
...
WHERE 
	...
	AND (RH.S_Receipt_No = ISNULL(@receiptNo, RH.S_Receipt_No) OR @receiptNo IS NULL)
	 AND (@isOnline IS NULL OR (@isOnline = 1 AND TM.I_ERP_Transaction_Master_ID IS NOT NULL) OR (@isOnline = 0 AND TM.I_ERP_Transaction_Master_ID IS NULL));
...
WHERE 
	...
	--select * from #Transaction_History
	  ...
	  -- Before the final SELECT, calculate the rows to skip and the rows to fetch
    DECLARE @RowStart INT = (@PageNumber - 1) * @PageSize;
    DECLARE @RowEnd INT = @PageNumber * @PageSize;


	 -- Replace your final SELECT with pagination
    WITH TransactionResults AS (
        SELECT 
            TH.*,
            ExternalStatus.StatusDescription,
            ExternalStatus.StatusColour,
            ROW_NUMBER() OVER (ORDER BY TH.TransactionDate DESC) AS RowNum -- Adjust sorting as needed
        FROM 
            #Transaction_History AS TH
        LEFT JOIN
            @PaymentStatusTable AS ExternalStatus ON TH.PaymentStatus = ExternalStatus.PaymentStatusID
        WHERE 
            TH.PaymentStatus = ISNULL(@iPaymentStatusID, TH.PaymentStatus)
