...
--EXEC [ERP_REPORTS].[usp_ERP_GetDueReport_HistoryNEW_Bak_27052025]  null,47,117,null,null,null,null,'2025-05-01','2025-05-27','ALL'          
          
CREATE    PROCEDURE [dbo].[usp_ERP_GetDueReport_History_API_W]                     
--EXEC [ERP_REPORTS].[usp_ERP_GetDueReport_HistoryNEW] 'null', 107, '2025-05-30', 'ALL'                    
    (                    
   ...
   @sessionID int=NULL,        
      --@iBrandID INT=NULL ,        
   @StrSchool_Group NVARCHAR(MAX)=NULL,        
   ...
   @insectionID NVARCHAR(MAX)=NULL,        
    ...
    @strstream NVARCHAR(MAX)=NULL,      
    @strsection NVARCHAR(MAX)=NULL,      
    @Page INT=1,      
    @lIMIT INT=10,      
...
AS                     
    BEGIN                    
      --SELECT  *                   
      --          FROM    dbo.fnGetCentreIdByBrandId(@iBrandID)        
      --SELECT @iBrandID=[I_Brand_ID] FROM [dbo].[T_Brand_Master]      
      --WHERE [tenant_ID]=@tenentId;      
        SET NOCOUNT ON ;                    
        DECLARE @offset INT=(@Page-1)* @Limit;              
        DECLARE @StudentStatus INT       
              
        SET @dtUptoDate=CONVERT(DATE,GETDATE());      
      ...
      -- select * from #tempinvoicedt              
        ...
        AND P2.I_Invoice_Header_ID IS NULL                    
         ...
         AND (C.Dt_Upd_On> ( SELECT TOP 1 T.Dt_Upd_On                     
               FROM                     
               #tempinvoicedt as T           
      Left Join C on T.I_Invoice_Header_ID=C.I_Invoice_Header_ID                    
               and C.I_Status=0                    
        ...
        (                    
          ...
          S_Mobile_No VARCHAR(50) ,                            S_Student_ID VARCHAR(100) ,                    
          I_Roll_No INT ,                    
  ...
  (                    
    ...
    Inv_SessionID        
        ...
        SELECT  B.I_Invoice_Child_Header_ID ,                    
              ...
              A.S_Invoice_No ,                                    A.Dt_Invoice_Date ,                    
 ...
 A.I_School_Session_ID        
        FROM    T_Invoice_Parent A                    
                ...
                Left JOIN #tempinvoice ON #tempinvoice.I_Invoice_Header_ID = A.I_Invoice_Header_ID                    
...
(A.I_Status=0 and C.I_Invoice_Detail_ID in(Select tc.I_Invoice_Detail_ID From T_Receipt_Component_Detail as tc                     
     ...
     and tc.I_Invoice_Detail_ID not in(Select I_Invoice_Detail_ID From T_Credit_Note_Invoice_Child_Detail)))                    
        ...
        WHERE   A.I_Centre_Id IN (        
                ...
                AND Dt_Installment_Date BETWEEN ISNULL(@fromdt, '1900-01-01') AND @dtUptoDate                    
    ...
    --            C.I_Fee_Component_ID,                    
    --            S_Component_Name ,                    
    --            C.N_Amount_Due ,                  --            Dt_Installment_Date ,                    
    --            I_Installment_No ,                    
    --            A.I_Parent_Invoice_ID ,                    
    --            A.Dt_Crtd_On ,                    
    --            C.I_Invoice_Detail_ID ,                    
  ...
  SELECT  B.I_Invoice_Child_Header_ID ,                    
   ...
   ,A.I_School_Session_ID               
                FROM    T_Invoice_Parent A                    
                        ...
                        INNER JOIN dbo.T_Brand_Center_Details AS tbcd ON tcm2.I_Centre_Id = tbcd.I_Centre_Id                    
                        INNER JOIN dbo.T_Brand_Master AS tbm ON tbcd.I_Brand_ID = tbm.I_Brand_ID                    
                        OUTER APPLY ( SELECT TOP 1                                                    tsbd.I_Student_Batch_ID ,                    
                                                tsbd.I_Status ,                    
                                                tsbd.I_Batch_ID                    
                                      FROM      dbo.T_Student_Batch_Details AS tsbd                    
                                      WHERE     tsbd.I_Student_ID = a.I_Student_Detail_ID                    
                        ...
                        LEFT JOIN dbo.T_Course_Master AS tcm ON tcm.I_Course_ID = tsbm.I_Course_ID                    
                        --LEFT JOIN T_Invoice_Detail_Tax F ON F.I_Invoice_Detail_ID = C.I_Invoice_Detail_ID                    
              WHERE   A.I_Centre_Id IN (SELECT fnCenter.centerID FROM dbo.fnGetCentreIdByBrandId(@iBrandID) fnCenter )                    
                        AND Dt_Installment_Date BETWEEN ISNULL(@fromdt, '1900-01-01') AND @dtUptoDate                    
                        AND E.I_Status = ISNULL(@StudentStatus, E.I_Status)                    
...
INNER JOIN #temphistory T ON RCD.I_Invoice_Detail_ID = T.I_Invoice_Detail_ID                    
  ...
  UPDATE #temphistory SET Total_Value = isnull(Due_Value,0) + isnull(Tax_Value,0)                    
        ...
        SET     T1.Amount_Paid = ISNULL(ROUND(T2.Amount_Paid, 2), 0)                    
        FROM    #temphistory T1                    
                INNER JOIN ( SELECT A.I_Invoice_Detail_ID ,                      SUM(N_Amount_Paid) Amount_Paid                    
 ...
 --     SELECT @TotalRecord= COUNT(*) FROM    #temp T                    
 --   INNER JOIN dbo.fnGetCentreIdByBrandId(@iBrandID) FN1 ON T.I_Center_ID = FN1.CenterID          
 --   Inner Join T_student_class_section scs on scs.S_Student_ID=T.S_Student_ID        
 --and scs.I_Status=1 and scs.I_Brand_ID=@iBrandID         
 --left join T_School_Group_Class sgc on sgc.I_School_Group_Class_ID=scs.I_School_Group_Class_ID        
 --left Join T_School_Group SG on SG.I_School_Group_ID=sgc.I_School_Group_ID and SG.I_Brand_Id=@iBrandID        
 --left Join T_Class tc on tc.I_Class_ID=sgc.I_Class_ID and tc.I_Brand_ID=@iBrandID        
 --Left Join T_Section ts on ts.I_Section_ID=scs.I_Section_ID        
 --Left Join T_Stream st on st.I_Stream_ID=scs.I_Stream_ID and st.I_brand_id=@iBrandID        
 ...
 -- WHERE ISNULL(Total_Due, 0)>0    and (T.Inv_SessionID=@sessionID  OR T.Inv_SessionID IS NULL)            
 -- AND sg.S_School_Group_Name=@StrSchool_Group                
 --     AND TC.S_Class_Name= @strclass       
 --     AND (@strstream IS NULL OR st.S_Stream=@strstream)        
 --     AND (@strsection IS NULL OR ts.S_Section_Name=@strsection);      
...
FROM (    
    ...
    LEFT JOIN T_Section TS ON TS.I_Section_ID = SCS.I_Section_ID    
    LEFT JOIN T_Stream ST ON ST.I_Stream_ID = SCS.I_Stream_ID AND ST.I_Brand_ID = @iBrandID    
    WHERE ISNULL(T.Total_Due, 0) > 0    
      AND (@sessionID IS NULL OR T.Inv_SessionID = @sessionID)    
      AND SG.S_School_Group_Name = @StrSchool_Group    
      AND TC.S_Class_Name = @strclass    
      AND (@strstream IS NULL OR ST.S_Stream = @strstream)    
      AND (@strsection IS NULL OR TS.S_Section_Name = @strsection)    
    ...
    --(      
 --   SELECT DISTINCT     --   T.I_Student_Detail_ID , T.S_Mobile_No ,T.S_Student_ID ,        
 --SG.S_School_Group_Name AS School_Programme,tc.S_Class_Name as Class,        
 --ts.S_Section_Name as Section,st.S_Stream AS Stream,        
 --T.I_Roll_No ,T.S_Student_Name ,T.S_Invoice_No ,T.S_Receipt_No ,                    
 --   T.Dt_Invoice_Date ,T.I_Fee_Component_ID,T.S_Component_Name ,T.S_Batch_Name ,T.S_Course_Name ,T.I_Center_ID ,                    
 ...
 -- ORDER BY S_Center_Name, S_Batch_Name, I_Roll_NO      
    ...
    SELECT DISTINCT        
    T.S_Mobile_No AS mobile,      
    T.S_Student_ID AS student_id,            
    SG.S_School_Group_Name AS academic_programme,      
    TC.S_Class_Name AS class_name,            
    TS.S_Section_Name AS section_name,      
    ST.S_Stream AS stream_name,            
    T.S_Student_Name AS student_name,      
    Convert(Numeric(12,2),SUM(T.Due_Value)) AS due_amount,      
    MAX(T.Dt_Installment_Date) AS due_date,                       
    SUM(T.Total_Value) AS Total_Value,      
    SUM(T.Total_Due) AS Total_Due  ,  
 1 AS StatusFlag   
FROM #temp T                        
INNER JOIN dbo.fnGetCentreIdByBrandId(@iBrandID) FN1       
    ON T.I_Center_ID = FN1.CenterID           
INNER JOIN T_student_class_section SCS       
    ON SCS.S_Student_ID = T.S_Student_ID            
    AND SCS.I_Status = 1       
    AND SCS.I_Brand_ID = @iBrandID             
...
LEFT JOIN T_School_Group SG       
    ...
    AND SG.I_Brand_Id = @iBrandID            
LEFT JOIN T_Class TC       
    ON TC.I_Class_ID = SGC.I_Class_ID       
    AND TC.I_Brand_ID = @iBrandID            
LEFT JOIN T_Section TS       
    ON TS.I_Section_ID = SCS.I_Section_ID            
LEFT JOIN T_Stream ST       
    ON ST.I_Stream_ID = SCS.I_Stream_ID       
    AND ST.I_Brand_ID = @iBrandID            
WHERE       
    ISNULL(T.Total_Due, 0) > 0          
    AND (@sessionID IS NULL OR T.Inv_SessionID = @sessionID)                
    AND SG.S_School_Group_Name = @StrSchool_Group                    
    AND TC.S_Class_Name = @strclass           
    AND (@strstream IS NULL OR ST.S_Stream = @strstream)            
