CREATE PROCEDURE [dbo].[usp_ERP_insert_Update_Fee_Plan_Structure_Component]          
   ...
   @p_Is_Late_Fine_Applicable bit,    
    ...
    @p_Is_Active bit,    
 @p_Action_By int,  
 @CollectionStartMonth int,  
 @CollectionEndMonth int,  
 @PaymentScheduleID int,  
 @Fee_ComponentDEtails UT_FeePlan_Structure_Component_Details READONLY   ,
 @ibrandiD int=NULL,
 @iFeeCategoryID int=NULL
...
BEGIN          
  ...
  DECLARE @ErrMessage NVARCHAR(4000) --,@BrandID int--,@sessionID   
  IF @ibrandiD is NULL 
  begin 
  ...
  begin
  set @ibrandiD=@ibrandiD
  end
     

	 if @iFeeCategoryID IS NULL OR 
	 exists
	 (	 
	 select 
	FCM.I_ERP_Fee_Category_Master_ID as FeeCategoryMasterID,FCM.S_Fee_Category_Name as FeeCategoryName,
	FCM.I_Fee_Category_FeeStructure_Capacity as FeeCategoryFeeStructureCapacity,
	ISNULL(UsedFeeCategory.UsedCount,0) as UsedCount
	from 
	T_ERP_Fee_Category_Master as FCM
	left join
	(
	select 
	FCM.I_ERP_Fee_Category_Master_ID,count(*) as UsedCount
	from T_ERP_Fee_Structure as EFS
	inner join
	T_ERP_Fee_Category_Master as FCM on EFS.I_ERP_Fee_Category_Master_ID=FCM.I_ERP_Fee_Category_Master_ID
	where EFS.I_Brand_ID=FCM.I_Brand_ID --and FCM.I_Status_ID=1	
	group by FCM.I_ERP_Fee_Category_Master_ID

	) UsedFeeCategory on FCM.I_ERP_Fee_Category_Master_ID=UsedFeeCategory.I_ERP_Fee_Category_Master_ID

	where FCM.I_Fee_Category_FeeStructure_Capacity > ISNULL(UsedFeeCategory.UsedCount,0)
	and FCM.I_ERP_Fee_Category_Master_ID=@iFeeCategoryID
	 )
...
from T_ERP_Fee_Structure fs
inner Join T_ERP_Fee_Structure_AcademicSession_Map fsm on fsm.I_Fee_Structure_ID=fs.I_Fee_Structure_ID
where fs.S_Fee_Structure_Name=@p_S_Fee_Structure_Name and fsm.I_Brand_ID=@ibrandiD)  
   ...
   BEGIN  
    ...
    INSERT INTO T_ERP_Fee_Structure_Installment_Component (          
     ...
     Select          
      ...
      [p_I_Action_By],          
    1  ,  
    CASE WHEN [p_Is_OneTime] = 'true' THEN [p_Collection_Start_Period_Month] ELSE NULL END,  
    [p_Collection_Start_Period_Month],  
    p_Collection_End_Period_Month  
    from @Fee_ComponentDEtails     
   
   END  
   ELSE  
    BEGIN  
  
     SELECT @ErrMessage='Same Fee Structure Name already exists'    
...
BEGIN      
   select 1 as StatusFlag,
          'Fee Structure Successfully Saved' as Message,
          @h_I_Fee_Structure_ID as Id
END
  ...
  END    
	...
	BEGIN
			...
			SELECT @ErrMessage='Invalid ! Fee Structure exceed highest Capacity '    
