--EXEC [ERP_Usp_batchCreation_N] 121628
...
Declare @EnquiryID  int            
   ...
   Declare @currSessionID int
   SET @currSessionID=(select R_I_School_Session_ID from T_Enquiry_Regn_Detail where I_Enquiry_Regn_ID=@EnquiryID)
...
SET @FinancialYearStart = DATEFROMPARTS(   CASE            
          ...
          YEAR(@CurrentDate)            
									   ...
									   --------------
SET @FinancialYearStart = (Select Convert(date,Dt_Session_Start_Date) from T_School_Academic_Session_Master 
where I_School_Session_ID=@currSessionID)
