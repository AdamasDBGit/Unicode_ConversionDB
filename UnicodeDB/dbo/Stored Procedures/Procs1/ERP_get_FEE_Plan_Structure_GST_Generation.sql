...
CREATE Proc [dbo].[ERP_get_FEE_Plan_Structure_GST_Generation](  
@FeeStructureID int,  
...
I_GST_FeeComponent_Catagory_ID  
 ...
 SELECT              
     ...
     ScheduledInstallments.ComponentActualTotalAnnualAmount,--TEFSIC.N_Component_Actual_Total_Annual_Amount,          
     TEFSIC.R_I_Fee_Component_ID,          
     TEFC.S_Component_Name,          
     TEFSIC.R_I_Fee_Pay_Installment_ID,          
     ScheduledInstallments.NumberofInstallment,-- TEFPT.I_Pay_InstallmentNo,          
   ...
   -- Into #GST_Component_Amount             
    from T_ERP_Fee_Structure TEFS              
		...
		INNER JOIN (
		select SUM(N_Component_Actual_Total_Annual_Amount) as ComponentActualTotalAnnualAmount,
		count(*) as NumberofInstallment
		,I_Fee_Structure_Installment_Component_ID  from T_ERP_Fee_Structure_Session_Installment_Breakup group by R_I_Fee_Component_ID,I_Fee_Structure_Installment_Component_ID
		) as ScheduledInstallments 
		on ScheduledInstallments.I_Fee_Structure_Installment_Component_ID=TEFSIC.I_Fee_Structure_Installment_Component_ID
 ...
 and TEFSIC.Is_OneTime= ISNULL(@paymentTypeID,TEFSIC.Is_OneTime) and b.Is_Active=1  
