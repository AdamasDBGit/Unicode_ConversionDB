...
-- exec [dbo].[usp_ERP_GetFeeComponent_Master] 1219, 107, null, null,NULL  
-- =============================================    
    
create   PROCEDURE [dbo].[usp_ERP_GetFeeComponent_Master_BKP_May_2025]    
 @FeeHeadId int null,    
 @iBrand int,    
 @isindividual bit = NULL,    
 @isadhoc bit =NULL,
 @FeeComponentName NVARCHAR(MAX)=NULL
...
(  
    SELECT   
        ...
        CASE WHEN TFCT.S_Fee_Component_Type_Name like '%Individual%' THEN 'Additional Component'    
  ...
  CONVERT(date, Dt_Valid_From) AS Valid_from,  
    ...
    LEFT JOIN   
		...
		T_ERP_GST_Component_Mapping AS TEGIC ON a.I_Fee_Component_ID = TEGIC.I_Fee_Component_ID
		and TEGIC.Is_Active=1 and TEGIC.I_GST_Component_Type=1
    LEFT JOIN   
    ...
    WHERE     
		...
		AND (@FeeComponentName IS NULL OR A.S_Component_Name like '%'+ISNULL(@FeeComponentName,A.S_Component_Name)+'%')
) AS Subquery  
WHERE  
    RowNum = 1  
ORDER BY   
