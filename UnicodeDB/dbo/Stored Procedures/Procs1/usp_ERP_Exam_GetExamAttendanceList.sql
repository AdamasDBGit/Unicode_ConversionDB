...
-- =============================================
--EXEC usp_ERP_Exam_GetExamAttendanceList NULL,NULL,null
CREATE PROC [dbo].[usp_ERP_Exam_GetExamAttendanceList] 
(   
    @SessionId INT = NULL,
...
BEGIN 
    ...
    WITH PAGED AS (
        SELECT
			...
			MIN(SSD.inAttendanceStatus) AS inAttendanceStatus,
        ...
        FROM 
			...
			JOIN T_ERP_Exam_Slot_Master SM ON SM.inSlotID= SSD.inExamSlotId
	...
	IF @ExamTypeId IS NOT NULL
        SET @stSQL = @stSQL + ' AND SD.inExamTypeId IN (SELECT CAST(Value AS INT) FROM dbo.SplitString(@ExamTypeId, '',''))'
    ...
    EXEC sp_executesql @stSQL,
        N'@SessionId INT, @SchoolGroupId INT, @SchoolClassId VARCHAR(200), @ExamName NVARCHAR(200), @ExamScheduleStatus INT, @ExamTypeId NVARCHAR(200), @ExamCategoryId NVARCHAR(200)',
