...
BEGIN      
    ...
    SELECT DISTINCT      
        SD.S_Student_ID, SD.S_First_Name, SD.S_Middle_Name, SD.S_Last_Name, SD.I_Enquiry_Regn_ID,      
    ...
    WHERE TID.ReceiptHeaderID IS NULL      
      ...
      AND CONVERT(DATE, TM.Dt_TransactionDate) BETWEEN CONVERT(DATE, @dtValidFrom) AND CONVERT(DATE, @dtValidTo)      
    ...
    SELECT       
        TH.*,      
        TH.S_First_Name + ' ' + ISNULL(TH.S_Middle_Name, '') + ' ' + TH.S_Last_Name AS StudentName,      
