CREATE PROCEDURE [dbo].[GetStudentsForPromotionFinancialApproval]
(
...
BEGIN
    ...
    /*
        ===================================================================
        Author:      Susmita Paul
        Created On:  2025-Oct-15
        Description: 
            Fetch students from T_ERP_Student_Promotion_History_Header 
            who are academically approved (IsAcademicApproved = 1),
            promoted (I_Promotion_Status = 1),
            but not yet financially approved (IsFinancialApproved IS NULL).
    ...
    */
    ...
    SELECT 
        ...
        PH.RollNo AS DestinationRollNo,  -- Assuming roll may be same; adjust if separate column exists
    ...
    WHERE 
        ...
        AND PH.IsAcademicApproved = 1
        AND PH.I_Promotion_Status = 1
        AND PH.IsFinancialApproved IS NULL
