...
IF @iBrandID=107 BEGIN set @sHierarchyList='53' END
  
        -- compute prev month date (kept exactly as original logic)  
     
set @dtPrevMonthDate = CASE WHEN DATEDIFF(MONTH, @dtEndDate,
                                                      DATEADD(DAY, 1,
                                                              @dtEndDate)) = 1
                                        THEN DATEADD(DAY, -1,
                                                     DATEADD(MONTH,
...
IF ( @sReconType = 'DUE' )
 ...
 -- datetime
        ...
        INSERT  INTO #ERPDUERECON
                ...
                SELECT DISTINCT
                        T1.S_Student_ID
                FROM    ( SELECT    D1.S_Student_ID
                          FROM      #DUERECONMONTH AS D1
                          UNION ALL
                          SELECT    D2.S_Student_ID
                          FROM      #DUEPREVMONTH AS D2
                          UNION ALL
                          SELECT    E.S_Student_ID
                          FROM      #ERPSYNC AS E
                        ) T1 
        ...
        FROM    #ERPDUERECON AS T1
                INNER JOIN ( SELECT D1.S_Student_ID ,
        ...
        FROM    #ERPDUERECON AS T1
                INNER JOIN ( SELECT E1.S_Student_ID ,
...
ELSE
 ...
 -- datetime 
            ...
            FROM    #ERPADVCAL AS T1
                    INNER JOIN ( SELECT E.S_Student_ID ,
                                        ISNULL(SUM(ISNULL(E.FinalAmount, 0.00)),
                                               0.00) AS Col
                                 FROM   #ERPSYNCADV AS E
                                 WHERE  E.I_Transaction_Nature_ID IN ( 3, 5, 7,
                                                              8 )
            ...
            UPDATE  #ERPADVCAL
            SET     ActAdv = ISNULL(CollectionData, 0.00) - ISNULL(AdjData,
                                                              0.00)
            ...
            FROM    #ERPADVRECON AS T1
                    INNER JOIN ( SELECT E.Std ,
                                        ISNULL(SUM(ISNULL(E.ActAdv, 0.00)),
                                               0.00) AS TotalAdv
                                 FROM   #ERPADVCAL AS E
                                 GROUP BY E.Std
