...
CREATE   PROCEDURE [dbo].[usp_ERP_GetRevisedFeePlanDetailsForOldInvoice_BKP_June_2025] --uspGetFeePlanDetailsForOldInvoice 168864, 37602
( 
...
DECLARE @Eligible bit = 'true'
	...
	select * from #FeePaid FP left join #FinalInstallment FI on FP.I_Fee_Component_ID=FI.Fee_ComponentID
	where FI.Fee_ComponentID IS NULL
	)
	BEGIN
		set @Eligible = 'false'
	END
...
DECLARE @Instamount DECIMAL(18, 2);
...
BEGIN
    ...
    BEGIN
		...
		PRINT CONCAT('Installment Date: ', @InstallmentDates, ', Component ID: ', @paidFee_ComponentID);
		PRINT CONCAT('Before: Remaining Amount: ', @RemainingAmount, ', Installment Amount: ', @InstallmentAmt);
        ...
        BEGIN
            ...
            SET Installment_Amt = 0,Installment_Amt_IncludeTAX=0
        ...
        BEGIN
            ...
            UPDATE #FinalRemainingReviseInstallment
            SET Installment_Amt = Installment_Amt - @RemainingAmount,Installment_Amt_IncludeTAX=Installment_Amt_IncludeTAX-@RemainingAmount
            WHERE Fee_ComponentID = @paidFee_ComponentID AND Installmentdt = @InstallmentDates;

            SET @RemainingAmount = 0;
        END;

		-- Fetch top 1 Installment_Amt into @Instamount
			SELECT TOP 1 @Instamount = Installment_Amt  
			FROM #FinalRemainingReviseInstallment
			WHERE Fee_ComponentID = @paidFee_ComponentID 
			AND Installmentdt = @InstallmentDates;

			-- Correct PRINT statement with explicit conversion
			PRINT 'After: Remaining Amount: ' + CAST(@RemainingAmount AS VARCHAR(20)) + 
				  ', Installment Amt: ' + CAST(@Instamount AS VARCHAR(20));
...
update #FinalRemainingReviseInstallment set Installment_Amt=0,Installment_Amt_IncludeTAX=0
