...
--exec [usp_ERP_GetStudentInstallmentPayableDetails] 107,'24-0334'
...
where i_brand_id=@iBrandID and I_School_Session_ID=@sessionID  
   ...
   set @studentStatus = (select top 1 I_Status from T_Student_Class_Section 
   where S_Student_ID=@sStudentID and I_School_Session_ID=@sessionID and I_Status=1)  
    print 'status'  
 print    @studentStatus       
...
Else NULL End
as Fee_Remarks,
ISNULL((@TotalFineHeader1+@TotalFinePaid),0) as TotalFinePayable,fsinv.Dt_Invoice_Date
...
,ivp.I_School_Session_ID,ivp.Dt_Invoice_Date
...
where  FSinv.I_School_Session_ID=@sessionID and p1.I_sessionID=@sessionID
order by fsinv.Dt_Invoice_Date desc
...
END
 as Freezed 
 ,Case When p4.i_invoice_Detail_ID=0 Then FST.FineDiscAmount 
 Else 
 ISNULL(ICCD.N_Disc_Amount,0) end as L3DiscAmt 
 
 ,Case When  p4.i_invoice_Detail_ID=0 Then FST.ActualFineAmount
 Else 
 
 CASE WHEN ICCD.N_Gross_Amount IS Null Then ISNULL(p4.PayableBaseAmount,0)
 ELSE 
 ISNULL(ICCD.N_Gross_Amount,0) end end as GrossAmount,
 ...
 ,
 ISNULL(p4.PayableBaseAmount,0) as ActualAmount
...
Left JOin #StudentFine FST ON FST.Temp_Inv_No=p4.InvoiceNo
