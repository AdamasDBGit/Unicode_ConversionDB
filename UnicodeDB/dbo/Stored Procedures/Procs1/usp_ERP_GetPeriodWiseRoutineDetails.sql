...
select DISTINCT a.*, b.Actual_Subject_Allocation,
CASE WHEN logbook.I_Routine_Structure_Detail_ID IS NULL THEN 'true'
ELSE 'false' END as IsEditable
from #GetPeriodWiseRoutineDetails a      
...
left join 
(
select SCR.I_Routine_Structure_Detail_ID,SCR.I_Subject_ID from T_ERP_Student_Class_Routine as SCR
inner join
T_ERP_Teacher_Time_Plan as TTP on SCR.I_Student_Class_Routine_ID=TTP.I_Student_Class_Routine_ID
) as logbook on logbook.I_Routine_Structure_Detail_ID=a.RoutineStructureID and logbook.I_Subject_ID=a.SubjectID
