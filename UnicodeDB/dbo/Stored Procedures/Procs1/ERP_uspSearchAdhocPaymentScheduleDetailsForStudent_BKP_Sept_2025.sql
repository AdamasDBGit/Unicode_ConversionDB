...
CREATE PROCEDURE [dbo].[ERP_uspSearchAdhocPaymentScheduleDetailsForStudent_BKP_Sept_2025]
(
    @StudentDetailID INT
)
AS
BEGIN
    ...
    (
        ...
        INNER JOIN T_ERP_AdhocPaymentScheduleHeaderDetail EAPSHD
                                                      ON EAPSHD.inAdhocPaymentScheduleHeaderDetailID = EAPSSD.inAdhocPaymentScheduleHeaderDetailID
        INNER JOIN T_ERP_AdhocPaymentScheduleHeader EAPSD
                                                      ON EAPSD.inAdhocPaymentScheduleHeaderID = EAPSHD.inAdhocPaymentScheduleHeaderID
        ...
        WHERE SD.I_Student_Detail_ID = @StudentDetailID
          AND EAPSSD.I_Receipt_Header_ID IS NULL
    )
    SELECT
        P.*,
    ...
    LEFT JOIN T_Status_Master SM
           ON SM.I_Status_Value = P.AdHocFeeComponentID AND SM.I_Brand_ID = P.BrandID
    LEFT JOIN [dbo].[T_ERP_Configuration_Master] ECM
           ON SM.I_ConFig_ID = ECM.I_Config_ID
    LEFT JOIN T_ERP_GST_Component_Mapping GIC
           ON GIC.I_Fee_Component_ID = SM.I_Status_Value AND GIC.I_GST_Component_Type = 2
    LEFT JOIN T_ERP_GST_Item_Category GICM
           ON GICM.I_GST_FeeComponent_Catagory_ID = GIC.I_GST_FeeComponent_Catagory_ID AND GICM.Is_Active = 1
    LEFT JOIN T_ERP_GST_Configuration_Details GICD
           ON GICD.I_GST_FeeComponent_Catagory_ID = GIC.I_GST_FeeComponent_Catagory_ID
    LEFT JOIN T_ERP_BankMaster EBM
           ON SM.BankID = EBM.ID AND EBM.inBrandID = P.BrandID
    ORDER BY P.DueDate DESC, P.InvoiceNo DESC;
END
