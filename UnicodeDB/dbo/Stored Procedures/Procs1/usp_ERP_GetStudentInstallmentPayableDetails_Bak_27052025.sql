...
--select * from T_Receipt_Header where S_Receipt_No=''

--EXEC [usp_ERP_GetStudentInstallmentPayableDetails] 107,'25-0405'
create PROCEDURE [dbo].[usp_ERP_GetStudentInstallmentPayableDetails_Bak_27052025]    
...
--select * from T_Student_Detail where S_Student_ID='25-0405'
  ...
  SET NOCOUNT ON  ;                    
   ...
   select Distinct top 1 @CentreID=B.I_Centre_Id ,@MobileNo=TPM.S_Mobile_No,@StudentDetailID=D.I_Student_Detail_ID from T_Student_Detail A                        
...
--Select @TotalFinePaid  
 ...
 ,0  as IGST_Per  
  Into #FineCalculate  
  from #LoopGSTCal   where DueType='Current'     
...
Update #PayableHeader1 set Adhoc_TotalAmount=@TotalFineHeader1,AdhocTotalPaidAmount=isnull(@TotalFinePaid,0),SummaryDueAmount= SummaryDueAmount+ISNULL(@TotalFineHeader1,0)        
Select @studentStatus Status,* from #PayableHeader1--------Showing 1          
  --Print '1-01'    
...
select DISTINCT p4.*,ISNULL(ICCD.is_Freezed,0) as Freezed from #finalDetails p4---showing 4          
