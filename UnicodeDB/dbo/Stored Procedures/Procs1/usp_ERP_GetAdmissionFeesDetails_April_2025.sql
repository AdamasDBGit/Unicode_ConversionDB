CREATE PROCEDURE [dbo].[usp_ERP_GetAdmissionFeesDetails_April_2025]            
(            
    @EnquiryID INT,            
    @Brand_ID INT = NULL,            
    @SessionID INT = NULL            
...
AS            
BEGIN            
    DECLARE @formcellid INT,  @PrevSessionID int      
 SET @PrevSessionID=(  
 select top 1 I_School_Session_ID from [dbo].[T_School_Academic_Session_Master]   
 where I_Brand_ID=107  
 and I_Current_Session=0 Order by Dt_Session_Start_Date desc  
 )  
 Declare @PrvstartDt date,@prevEnddt date  
  
 Select @PrvstartDt=Dt_Session_Start_Date,@prevEnddt=Dt_Session_End_Date  
 from T_School_Academic_Session_Master where I_School_Session_ID=@PrevSessionID  
...
select top 1 @StudentDetailID = I_Student_Detail_ID from T_Student_Detail where I_Enquiry_Regn_ID=@EnquiryID    
 ...
 OR t1.I_Student_Detail_ID = @StudentDetailID)     
 and t2.Is_AdmissionToken = 'true'    
 and t1.I_Status=1  and convert(date,t1.Dt_Receipt_Date)>= @PrvstartDt   
 and convert(date,t1.Dt_Receipt_Date)<= @prevEnddt  
