CREATE   PROCEDURE [dbo].[uspInsertReceiptDetailsFromAPI]            
(            
@sReceiptDetail XML ,            
...
WHILE ( @InnerPosition <= @InnerCount )            
    BEGIN            
        ...
        FROM    ( SELECT    T1.* ,            
                    ...
                    FROM      ( SELECT    TICD.I_Invoice_Detail_ID ,            
                                        TIDT.I_Tax_ID ,            
                                        TIDT.N_Tax_Value ,            
                                        TTCFC.N_Tax_Rate            --CAST(( TTCFC.N_Tax_Rate / 100 ) * 1250 AS DECIMAL(18, 2)) AS PaidTaxAPI            
                    ...
                    AS TIDT ON TIDT.I_Invoice_Detail_ID = TICD.I_Invoice_Detail_ID            
                                        Left JOIN dbo.T_Tax_Country_Fee_Component            
  ...
  IF (@nTaxPaid<>0)                    
        ...
        SELECT  @iReceiptComponentDetailId,TTax.I_Tax_ID,TTax.I_Invoice_Detail_ID,CAST(@nTaxPaid AS FLOAT) / 2,CAST(@nTaxPaid AS FLOAT) / 2         
...
--CAST(( TTCFC.N_Tax_Rate / 100 ) * 1250 AS DECIMAL(18, 2)) AS PaidTaxAPI            
            ...
            END             
             ------Commented out for partial tax payment                        
...
@ErrorState = ERROR_STATE();          
--INSERT INTO ERP_ErrorLogTable (ErrorMessage, ErrorSeverity, ErrorState, ErrorProcedure)          
--VALUES (@ErrorMessage, @ErrorSeverity, @ErrorState, 'uspInsertReceiptDetailsFromAPI');        --Insert into tEst(Test) Values('After Log10')          
