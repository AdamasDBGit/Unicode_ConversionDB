...
set @start = (select top 1 Dt_Session_Start_Date FROM T_School_Academic_Session_Master WHERE I_Current_Session = 1 and I_Brand_ID=@iBrandID order by I_School_Session_ID desc)
set @end = (select top 1 Dt_Session_End_Date FROM T_School_Academic_Session_Master WHERE I_Current_Session = 1 and I_Brand_ID=@iBrandID order by I_School_Session_ID desc)
SELECT 
AM.*,
TE.I_Event_ID ID 
