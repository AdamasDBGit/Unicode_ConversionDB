...
CREATE PROCEDURE [ERP].[uspPrepareStudentFeeAccrualData_Arivoo] --exec [ERP].[uspPrepareStudentFeeAccrualData] '2017-07-07'
(
...
BEGIN
	 ...
	 DECLARE @CGSTID INT=7
	 DECLARE @SGSTID INT=8
	 DECLARE @IGSTID INT=9


	 select @CGSTID=I_Tax_ID from T_Tax_Master where S_Tax_Code='CGST' and I_Status=1
	 select @SGSTID=I_Tax_ID from T_Tax_Master where S_Tax_Code='SGST' and I_Status=1
	 select @IGSTID=I_Tax_ID from T_Tax_Master where S_Tax_Code='IGST' and I_Status=1
	...
	SELECT	tbcd.I_Brand_ID, tttm.I_Transaction_Type_ID, tip.I_Student_Detail_ID,NULL,
			...
			(
			--- susmita : Revam  for arivoo--
			select TaxRate as N_Tax_Rate from
			(
			 select GIC.I_Fee_Component_ID,GCD.N_Start_Amount,GCD.N_End_Amount,GCD.N_SGST as TaxRate,@SGSTID I_Tax_ID
			 ...
			 where ISNULL(N_SGST,0) > 0 and GIC.I_Fee_Component_ID IS NOT NULL and  @dtExecutionDate between GCD.dt_ValidFrom_dt and ISNULL(GCD.dt_ValidTo_dt,GETDATE())
			 and (ROUND(
                        ABS(ISNULL((ISNULL(ticd.N_Amount_Due, 0) - ISNULL(ticd.N_Discount_Amount, 0)), 0)), 0) - 
                        ROUND(ABS(ISNULL(A.N_Amount_Paid, 0)), 0)
                    ) between GCD.N_Start_Amount and GCD.N_End_Amount
			 union
			 select GIC.I_Fee_Component_ID,GCD.N_Start_Amount,GCD.N_End_Amount,GCD.N_CGST as TaxRate,@CGSTID I_Tax_ID
			 from T_ERP_GST_Item_Category as GIC 
			 inner join
			 T_ERP_GST_Configuration_Details as GCD on GIC.I_GST_FeeComponent_Catagory_ID=GCD.I_GST_FeeComponent_Catagory_ID
			 where ISNULL(N_CGST,0) > 0 and GIC.I_Fee_Component_ID IS NOT NULL and  @dtExecutionDate between GCD.dt_ValidFrom_dt and ISNULL(GCD.dt_ValidTo_dt,GETDATE())
			 and (ROUND(
                        ABS(ISNULL((ISNULL(ticd.N_Amount_Due, 0) - ISNULL(ticd.N_Discount_Amount, 0)), 0)), 0) - 
                        ROUND(ABS(ISNULL(A.N_Amount_Paid, 0)), 0)
                    ) between GCD.N_Start_Amount and GCD.N_End_Amount
			 union
			  select GIC.I_Fee_Component_ID,GCD.N_Start_Amount,GCD.N_End_Amount,GCD.N_IGST as TaxRate,@IGSTID I_Tax_ID
			 from T_ERP_GST_Item_Category as GIC 
			 inner join
			 T_ERP_GST_Configuration_Details as GCD on GIC.I_GST_FeeComponent_Catagory_ID=GCD.I_GST_FeeComponent_Catagory_ID
			 where ISNULL(N_IGST,0) > 0 and GIC.I_Fee_Component_ID IS NOT NULL and  @dtExecutionDate between GCD.dt_ValidFrom_dt and ISNULL(GCD.dt_ValidTo_dt,GETDATE())
			 and (ROUND(
                        ABS(ISNULL((ISNULL(ticd.N_Amount_Due, 0) - ISNULL(ticd.N_Discount_Amount, 0)), 0)), 0) - 
                        ROUND(ABS(ISNULL(A.N_Amount_Paid, 0)), 0)
                    ) between GCD.N_Start_Amount and GCD.N_End_Amount
			) tcfc where 
			tcfc.I_Fee_Component_ID = ticd.I_Fee_Component_ID and tidt.I_Tax_ID=tcfc.I_Tax_ID
			--- susmita : Revam  for arivoo--

			--Select tcfc.N_Tax_Rate From T_Tax_Country_Fee_Component as tcfc where 
			--tcfc.I_Fee_Component_ID = ticd.I_Fee_Component_ID and tidt.I_Tax_ID=tcfc.I_Tax_ID and @dtExecutionDate between Dt_Valid_From and Dt_Valid_To
			...
			(
			 ...
			 and  ROUND(
                    ABS(
                        ISNULL(
                            ISNULL(ticd.N_Amount_Due, 0) - ISNULL(ticd.N_Discount_Amount, 0), 
                            0
                        )
                    ), 
                    0
	...
	(
	 select 
	 I_Invoice_Detail_ID,I_Invoice_Child_Header_ID,I_Fee_Component_ID,N_CGST as Taxamount,@CGSTID I_Tax_ID
	 from T_Invoice_Child_Detail as CGSTTAX where ISNULL(N_CGST,0) > 0
	 union
	 select 
	 I_Invoice_Detail_ID,I_Invoice_Child_Header_ID,I_Fee_Component_ID,N_SGST as Taxamount,@SGSTID I_Tax_ID
	 from T_Invoice_Child_Detail as SGSTTAX where ISNULL(N_SGST,0) > 0
	 union
	 select 
	 I_Invoice_Detail_ID,I_Invoice_Child_Header_ID,I_Fee_Component_ID,N_IGST as TaxAmount,@IGSTID I_Tax_ID
	 from T_Invoice_Child_Detail as IGSTTAX where ISNULL(N_IGST,0) > 0
	) tidt on ticd.I_Invoice_Detail_ID= tidt.I_Invoice_Detail_ID
	-- end of Susmita 
	--INNER JOIN dbo.T_Invoice_Detail_Tax AS tidt ON ticd.I_Invoice_Detail_ID = tidt.I_Invoice_Detail_ID
	LEFT JOIN (
...
--- susmita : Revam  for arivoo--
	...
	INNER JOIN dbo.T_Invoice_Child_Header AS tich ON tip.I_Invoice_Header_ID = tich.I_Invoice_Header_ID	
	INNER JOIN dbo.T_Invoice_Child_Detail AS ticd ON tich.I_Invoice_Child_Header_ID = ticd.I_Invoice_Child_Header_ID
	-- susmita started : 2024-Aug Revam for new Environment
	inner join
	(
	 select 
	 I_Invoice_Detail_ID,I_Invoice_Child_Header_ID,I_Fee_Component_ID,N_CGST as Taxamount,@CGSTID I_Tax_ID
	...
	-- end of Susmita : 2024-Aug Revam for new Environment
