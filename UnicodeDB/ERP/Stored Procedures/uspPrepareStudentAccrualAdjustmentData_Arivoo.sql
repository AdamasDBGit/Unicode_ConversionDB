...
CREATE PROCEDURE [ERP].[uspPrepareStudentAccrualAdjustmentData_Arivoo] --exec [ERP].[uspPrepareStudentAccrualAdjustmentData] '2017-07-29'
(
...
BEGIN
	...
	SELECT	tbcd.I_Brand_ID, tttm.I_Transaction_Type_ID, trh.I_Student_Detail_ID,trh.I_Enquiry_Regn_ID,
			tcm.S_Cost_Center, CASE WHEN ABS(tidx.Taxamount)> ABS(trtd.N_Tax_Paid) THEN ABS(trtd.N_Tax_Paid)-- replace column name  N_Tax_Value by new tax column
									ELSE ABS(tidx.Taxamount)
							   END, @dtExecutionDate, NULL, NULL
	FROM dbo.T_Receipt_Header AS trh
	INNER JOIN dbo.T_Receipt_Component_Detail AS trcd ON trh.I_Receipt_Header_ID = trcd.I_Receipt_Detail_ID
	...
	-- susmita  started : Tax_Value_Scheduled from new environment
	inner join
	(
	select 
	 I_Invoice_Detail_ID,I_Invoice_Child_Header_ID,I_Fee_Component_ID,N_CGST as Taxamount,@CGSTID I_Tax_ID,N_CGST_Tax_Value_Scheduled as N_Tax_Value_Scheduled
	 from T_Invoice_Child_Detail as CGSTTAX where ISNULL(N_CGST,0) > 0
	 union
	 select 
	 I_Invoice_Detail_ID,I_Invoice_Child_Header_ID,I_Fee_Component_ID,N_SGST as Taxamount,@SGSTID I_Tax_ID,N_SGST_Tax_Value_Scheduled as N_Tax_Value_Scheduled
	 from T_Invoice_Child_Detail as SGSTTAX where ISNULL(N_SGST,0) > 0
	 union
	 select 
	 I_Invoice_Detail_ID,I_Invoice_Child_Header_ID,I_Fee_Component_ID,N_IGST as TaxAmount,@IGSTID I_Tax_ID,I_GST_Tax_Value_Scheduled as N_Tax_Value_Scheduled
	 from T_Invoice_Child_Detail as IGSTTAX where ISNULL(N_IGST,0) > 0
	) AS tidx ON ticd.I_Invoice_Detail_ID = tidx.I_Invoice_Detail_ID AND tidx.I_Tax_ID=trtd.I_Tax_ID
	-- end of Susmita
	...
	SELECT	tbcd.I_Brand_ID, tttm.I_Transaction_Type_ID, trh.I_Student_Detail_ID,trh.I_Enquiry_Regn_ID,
			tcm.S_Cost_Center, CASE WHEN ABS(ISNULL(tidx.N_Tax_Value_Scheduled,0))> ABS(trtd.N_Tax_Paid) THEN ABS(trtd.N_Tax_Paid)
									ELSE ABS(ISNULL(tidx.N_Tax_Value_Scheduled,0))
							   END, @dtExecutionDate, NULL, NULL
	FROM dbo.T_Receipt_Header AS trh
	INNER JOIN dbo.T_Receipt_Component_Detail AS trcd ON trh.I_Receipt_Header_ID = trcd.I_Receipt_Detail_ID
	INNER JOIN dbo.T_Receipt_Tax_Detail AS trtd ON trcd.I_Receipt_Comp_Detail_ID = trtd.I_Receipt_Comp_Detail_ID
	...
	select 
	 ...
	 I_Invoice_Detail_ID,I_Invoice_Child_Header_ID,I_Fee_Component_ID,N_IGST as TaxAmount,@IGSTID I_Tax_ID,I_GST_Tax_Value_Scheduled as N_Tax_Value_Scheduled
	 from T_Invoice_Child_Detail as IGSTTAX where ISNULL(N_IGST,0) > 0
	) tidx on ticd.I_Invoice_Detail_ID= tidx.I_Invoice_Detail_ID AND tidx.I_Tax_ID=trtd.I_Tax_ID
	-- end of Susmita
	...
	SELECT	tbcd.I_Brand_ID, tttm.I_Transaction_Type_ID, trh.I_Student_Detail_ID,trh.I_Enquiry_Regn_ID,
			tcm.S_Cost_Center, CASE WHEN ABS(tidx.Taxamount)> ABS(trtd.N_Tax_Paid) THEN ABS(trtd.N_Tax_Paid)  -- susmita  started : Tax_Value from new environment
			ELSE ABS(tidx.Taxamount)
			END, @dtExecutionDate, NULL, NULL
	FROM dbo.T_Receipt_Header_Archive AS trh
	INNER JOIN dbo.T_Receipt_Component_Detail_Archive AS trcd ON trh.I_Receipt_Header_ID = trcd.I_Receipt_Detail_ID
	...
	SELECT	tbcd.I_Brand_ID, tttm.I_Transaction_Type_ID, trh.I_Student_Detail_ID,trh.I_Enquiry_Regn_ID,
			tcm.S_Cost_Center, CASE WHEN ABS(tidx.Taxamount)> ABS(trtd.N_Tax_Paid) THEN ABS(trtd.N_Tax_Paid) -- susmita  started : N_Tax_Value from new environment
									ELSE ABS(tidx.Taxamount)
							   END, @dtExecutionDate, NULL, NULL
	FROM dbo.T_Receipt_Header AS trh
	INNER JOIN dbo.T_Receipt_Component_Detail AS trcd ON trh.I_Receipt_Header_ID = trcd.I_Receipt_Detail_ID
	...
	SELECT	tbcd.I_Brand_ID, tttm.I_Transaction_Type_ID, trh.I_Student_Detail_ID,trh.I_Enquiry_Regn_ID,
			tcm.S_Cost_Center, CASE WHEN ABS(tidx.N_Tax_Value_Scheduled)> ABS(trtd.N_Tax_Paid) THEN ABS(trtd.N_Tax_Paid) --Tax_Value_Scheduled from new environment
