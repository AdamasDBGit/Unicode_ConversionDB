...
CREATE PROCEDURE [REPORT].[uspGetERPSyncReportDetail_BKP_Sept_2025]  
    (  
...
AS   
    ...
    INNER JOIN dbo.T_Centre_Master AS tcm ON tstd.S_Cost_Center = tcm.S_Cost_Center  
        ...
        WHERE   tstd.I_Enquiry_Regn_ID IS NULL  
                ...
                AND tcm.I_Centre_Id IN (select distinct I_Centre_Id from T_Brand_Center_Details where I_Brand_ID=@iBrandID)--(SELECT fgcfr.centerID FROM dbo.fnGetCentersForReports(@sHierarchyList, @iBrandID) AS fgcfr)  
