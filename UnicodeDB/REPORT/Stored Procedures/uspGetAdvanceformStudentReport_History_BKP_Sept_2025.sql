...
CREATE PROCEDURE [REPORT].[uspGetAdvanceformStudentReport_History_BKP_Sept_2025] --exec [REPORT].[uspGetAdvanceformStudentReport_History] '93',111,'2017-07-31','ALL','2011-01-01'
(
...
AS 
    BEGIN
        ...
        INSERT  INTO #temp
                ...
                WHERE   A.I_Centre_Id IN (select distinct I_Centre_Id from T_Brand_Center_Details where I_Brand_ID=@iBrandID)
				--(
  ...
  --                         ) T2 ON T1.I_Invoice_Detail_ID = T2.I_Invoice_Detail_ID
    ...
    --                                                   @iBrandID) fnCenter )
        ...
        SELECT  T.* ,DATENAME(MONTH,T.Dt_Installment_Date)+' '+CAST(DATEPART(YYYY,T.Dt_Installment_Date) AS VARCHAR) AS MonthYear,
               'AIS' --FN2.instanceChain
        FROM    #temp T
                INNER JOIN (select distinct I_Centre_Id from T_Brand_Center_Details where I_Brand_ID=@iBrandID)--[dbo].[fnGetCentersForReports](@sHierarchyList,
                --                                          @iBrandID) 
														  FN1 
														  ON T.I_Center_ID = FN1.I_Centre_Id
