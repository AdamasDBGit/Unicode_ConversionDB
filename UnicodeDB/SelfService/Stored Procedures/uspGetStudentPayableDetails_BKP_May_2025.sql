...
CREATE PROCEDURE [SelfService].[uspGetStudentPayableDetails_BKP_May_2025](@iBrandID INT,@StudentID VARCHAR(MAX))  
AS  
BEGIN  
   ...
   SET @FIYear=SUBSTRING(@FIYear,0,5)  
    ...
    TICD.N_Amount_Due,  
    CASE 
        WHEN YEAR(GETDATE()) = @FIYear THEN 'Current' 
        ELSE 'Previous' 
    END AS AcademicSession 
	Into #INVDET1
