...
--EXEC [SelfService].[uspGetStudentPayableDetails] 107,'24-0308'
CREATE   PROCEDURE [SelfService].[uspGetStudentPayableDetails](@iBrandID INT,@StudentID VARCHAR(MAX))    
AS    
BEGIN    
    
   --   DECLARE @iBrandID INT=107    
   --DECLARE @StudentID VARCHAR(MAX)='25-0084'    
   ...
   SET @FIYear=SUBSTRING(@FIYear,0,5)    
     ...
     AcademicSession VARCHAR(MAX)    
      ...
      --AcademicSession    
      --          )    
                SELECT       TCHND.I_Center_ID,     TCHND.S_Center_Name,       CASE          WHEN TCM2.S_Center_Code LIKE 'IAS T%' THEN 'IAS'           WHEN TCM2.S_Center_Code LIKE 'Judiciary T%' THEN 'Judiciary'           WHEN TCM2.S_Center_Code = 'BRST'
 THEN 'AIPT'           WHEN TCM2.S_Center_Code LIKE 'FR-%' THEN 'Franchise'           ELSE 'Own'       END AS TypeofCentre,       TCM.S_Course_Name,       TSBM.S_Batch_Name,       TSD.S_Student_ID,       TSD.S_First_Name + ' ' + ISNULL(TSD.S_Middle_Name, 
'') + ' ' + TSD.S_Last_Name AS StudentName,       TSD.S_Mobile_No AS ContactNo,       TSD.I_RollNo,       TIP.I_Invoice_Header_ID,       TICD.S_Invoice_Number,       TIP.Dt_Invoice_Date,       TICD.I_Invoice_Detail_ID,       TICD.I_Installment_No,      
TICD.I_Sequence,       TICD.Dt_Installment_Date,       TICD.I_Fee_Component_ID,       TFCM.S_Component_Name,       TICD.N_Amount_Due,       CASE          WHEN YEAR(GETDATE()) = @FIYear THEN 'Current'          ELSE 'Previous'      END AS AcademicSession   
 Into #INVDET1  
...
select DISTINCT S_Student_ID,StudentName,ContactNo,I_Centre_ID,S_Center_Name from #INVDET    
...
select DISTINCT S_Student_ID,I_Invoice_Header_ID as FeeScheduleID,I_Invoice_Detail_ID,S_Invoice_No,I_Installment_No,Dt_Installment_Date,I_Sequence,I_FeeComponent_ID,    
